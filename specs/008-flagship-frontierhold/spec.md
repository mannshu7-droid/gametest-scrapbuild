...
## サイクル14・1回目（BUILD+REVIEW相当）: single-stat all-in診断ボットのskill購入リーク修正と2つの着眼点の再検証

cycle13-finalが提案した2つの着眼点——(1) drill-all-inだけが持つ死亡リスクの非対称性は
「意図的な設計」か「見過ごされた偏り」か、(2) atk/hp/atkspeed/muffler系all-inのavgScoreが
359.8〜397.6の狭いレンジに収束している点——の両方に着手した。`src/core/game.ts`（ゲーム本体）
は無変更、`headless/simulate.ts`（検証ボット）のみを修正した。

### 発見: single-stat all-in戦略の「単一カテゴリ隔離」が実は不完全だった

cycle13-finalの着眼点(2)を調査する過程で、`Bot.tryBuy()`の「skill Lv1を最優先で確保する」
特例（005由来、skillUses=0問題への対処として全戦略共通で導入されたもの）が、cycle11で
single-stat all-in戦略を追加した際に除外されていないことを発見した。同じ関数内の「drill初回
購入だけは reserve を無視できる」特例は、cycle11導入時に`!singleStatOf(this.strategy)`で
single-stat all-in戦略を明示的に除外しているが、skill側にはこのガードが無かった。結果として
`atk-all-in`や`capacity-all-in`など「対象カテゴリ以外は一切買わない」はずの戦略が、実際には
skill Lv1も密かに購入していた（20シードの再検証で判明: 修正前はcapacity-all-in
avgSkillUses=12.3・fuel-all-in=6.2など、優先度リストに`skill`が一切含まれない戦略でも
skillが使われていた）。これはcycle11-v1・cycle13-finalの両方の「single-stat all-in」比較に
共通して存在した測定上のバイアスであり、cycle13-finalが問いかけた「見過ごされた偏り」は
ゲームバランス側ではなく検証ボット側に実在した。

`headless/simulate.ts`の該当箇所に`!singleStatOf(this.strategy)`ガードを追加し、single-stat
all-in戦略はskill早期購入の対象外にした（skill-all-in自身は元々`priorityFor`の先頭がskillのため
無関係）。既存8戦略（mining-first等）はこの特例を引き続きフルに使うため無変更。

### 修正前後の20シード（1〜20）比較

| 戦略 | avgScore（修正前→後） | avgMaxDepth（前→後） | deaths（前→後） |
|---|---|---|---|
...
既存8戦略（mining-first/combat-first/balanced/bridge-reliant/balanced-no-outpost/
3種のadaptive）は修正の影響を受けない設計のため、全指標が修正前と完全一致することを確認した
（回帰なし）。

### 着眼点(2)への回答: 「狭いレンジ」は5シードのサンプリング誤差だった

cycle13-finalが指摘した359.8〜397.6という狭いレンジは、8戦略×**5シード**（seed1〜5）だけの
平均値だった。今回、既にcycle11-v1が構築していた20シードの枠組みで再検証したところ、
修正前の時点で既にatk-all-in 358.1〜muffler-all-in 699.9という約2倍の開きがあり、skill購入
リークを修正した後もatk-all-in 353.6〜fuel-all-in 766.2（drill-all-inを除いても約2.2倍）と、
狭いレンジという前提そのものが20シードでは再現しなかった。5シードでは平均が偶然近い値に
収束しただけで、これはサンプルサイズ不足による統計的な誤差であり、アップグレードカテゴリ間の
投資価値が構造的に均質化しているという設計上の問題ではないと判断した。cycle11-v1が
既に指摘していた「迂回橋の都度払いが単一カテゴリの直接効果を薄め、drill以外の7〜8カテゴリの
avgMaxDepthを50〜63という近い値へ収束させる」という間接効果は今回も再現したが、avgScoreは
maxDepth以外の要素（生存・kills・moneyEarned等）も加味するため、maxDepthが近くてもスコアは
大きく開くという整合的な説明が付いた。

### 着眼点(1)への回答: drillの死亡リスク非対称性は「意図的な設計」、かつ従来の測定より強い

skill購入リークの修正により、drill-all-inの死亡率は55%(11/20)→**80%(16/20)**へさらに上昇した
（skillが「打たれ弱さ」を薄めていた分がなくなり、drillだけに全振りした際の真の脆弱性が
露わになった）。一方、死んだ16シードすべてがdamageTaken 96〜394という戦闘ダメージによる
死亡で、生き残った4シードのうち2つ（seed2, seed6）はband2(y≈39〜48)付近で足踏みしたまま
20000tick経過した「実質潜行を諦めた」個体、残る2つ（seed4, seed7)はdamageTaken34・495と
被弾がたまたま少なかった幸運な個体だった。つまりdrill-all-inが実際に深く潜り続けた場合、
ほぼ確実に戦闘で死ぬ。

ただしこれは「1つのカテゴリだけに全額投資し、他8カテゴリには1円も使わない」という現実の
プレイヤーが選ばない極端な診断用戦略でのみ現れる。実際の優先度リスト全体（mining-first等）で
確認すると、mining-first(drillを最優先だがatk/hpも並行投資)の死亡率は10/20(50%)、
balancedは5/20(25%)、3種のadaptive戦略（危険度ヒントに反応してhp優先を繰り上げる）は
0〜3/20(0〜15%)と、drill偏重の度合いが下がるほど死亡率は一貫して下がる。この勾配は
「drillだけが深さと引き換えの死亡リスクを持つ独立した進行手段」という構造が、複数カテゴリへの
現実的な配分の中では緩やかなトレードオフとして機能し、極端な全振りでのみ致命的になるという、
`combatRiskLevel`ヒント（008 v2導入）が想定していた設計意図と一致する。cycle11-v1の判定
（「意図通りのトレードオフ、実際のプレイヤーが選ぶバランス型のビルドでは発生しない」）を、
より正確な測定値（55%ではなく80%）とmining-first/balanced/adaptiveの勾配という新しい根拠で
再確認・補強した。`game.ts`側の変更は不要と判断した。

### ブラウザAIPでの検証

本回の変更は`headless/simulate.ts`のBot（Node専用の検証ボット）のみで、`src/core/game.ts`は
一切変更していないため、ブラウザ実行パスに影響する変更はない。cycle11-v1で既に
「ヘッドレスとブラウザの実行結果は完全一致する」ことを確認済みであり、本回は`npm run build`・
`npm run simulate`の正常終了のみで十分と判断し、追加のブラウザAIP再生検証は行わなかった。

### 判定

新たな致命的バランス崩壊は見つからなかった。検証ボット側の測定バイアス（skill購入リーク）を
...
## 見た目強化（ユーザー指示、自動ルーチンの回数外の作業）

これまでの描画は`fillRect`によるベタ塗り四角形（いわゆる「ドット絵未満」の原始的なプレース
ホルダー表現）だった。ユーザーから「本格的な2D/3Dゲームとして一般受けする見た目にしたい」との
指示を受け、以下の方針でsrc/render/のみを差し替えた（`src/core/`のゲームロジックは無変更）。

### 方針

- **2D強化を選択**（3D化はグリッド型メカニクス（採掘シャフト・ウェーブ戦闘・積み上げ建築）との
  相性が悪く、3Dモデル素材のコストが跳ね上がるため見送り）
- **キャラ・タイル等は画像スプライト、エフェクトはコード描画の併用**。画像生成AIツールが本環境に
  無いため、Kenney.nl（https://kenney.nl）の"Tiny Dungeon"アセットパック（CC0ライセンス、
  商用・改変とも許諾不要）から8枚のスプライトを選び`public/sprites/`に格納した（クレジットは
  `public/sprites/CREDITS.txt`参照）。エンジンは新規導入せず既存のCanvas 2D（`drawImage`）の
  ままとし、core/render分離のアーキテクチャは維持
- 適用範囲は**本命ゲーム(008)から**。他ゲーム(002〜007)・自動ルーチンの技術規約（CLAUDE.md）
  自体は今回は変更していない

### 実装

- `src/render/sprites.ts`（新規）: スプライト画像のプリロード・参照ユーティリティ。
  `public/sprites/*.png`をルート相対パスで読み込み、未読み込み・失敗時はnullを返す
  （呼び出し側が単色フォールバックへ切り替える）
- `src/render/renderer.ts`: タイル・敵・プレイヤーの`fillRect`単色描画を`drawImage`スプライト
  描画へ置き換え。鉱石3種(ORE_COPPER/IRON/GOLD)は岩スプライトを土台に、種類ごとの色の鉱脈ドット
  をコード描画で重ねて区別（画像1種を鉱石3種で共用しつつ視認性を確保）。プレイヤーのdash・
  moveEvasion状態は、スプライトの下に半透明グローを敷く形でコード描画のエフェクトとして維持
  （スプライト自体の色は変えない）。`ctx.imageSmoothingEnabled = false`でドット絵を拡大しても
  ぼやけないようにした
- スプライトマッピング: player(騎士)・enemy_crawler(緑スライム)・enemy_brute(褐色オーガ)・
  tile_dirt(土)・tile_rock(石レンガ、鉱石の土台も兼ねる)・tile_floor(砂色の掘削済み床)・
  tile_prop(支保工)・tile_outpost(宝箱=前線基地)

### 検証

- `npm run build`: 正常終了
- `npm run simulate`（20シード×17戦略）: 全指標がcycle14-v2と完全一致（回帰なし。描画層のみの
  変更でBotの意思決定・ゲームロジックには一切影響しないことを裏付けた）
- ブラウザプレビューは本環境の制約で起動できないため、実際のGameStateスナップショット
