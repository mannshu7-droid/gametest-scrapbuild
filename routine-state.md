...
- サイクル: 25
- 要素: **建築（building）**。対象ゲーム番号: **019-building-hallmark**（サイクル25・1回目で新規実装、
  2回目でFIX+REVIEW完了）
- 次に行う回: **3回目（FIX only、レビューなし・修正内容はPR本文とspec.mdに記載）**
- **サイクル25・2回目（reviews/019-building-hallmark-v2.md）からの引き継ぎ事項（3回目で対応検討）**:
  1. **【軽微・新規】careful（鑑定＋brace連携）がbraced（brace連携のみ、鑑定なし）をわずかに
     下回るようになった**（avgScore: v1は careful 1353.8 > braced 1304.7(+3.8%) だったが、
     v2は careful 2102.75 < braced 2201.15(-4.5%)）。brace連携が強力すぎて鑑定投資の効果が
     誤差程度に埋もれ、bracedが資材購入に全額回せる分マイルストーン報酬の資金雪だるま効果で
     わずかに先行する。対応候補: 鑑定済みロットの設置に少額のスコア/効率ボーナスを追加する等。
     致命ではなく（僅差）、対応必須ではないが3回目・4回目で余裕があれば検討すること
  2. **【軽微・継続】informedがsloppyをavgScore(+11.8%)・avgCollapseEvents(-20%)・avgBlocksLost(-20%)
     で明確に上回るようになった一方、avgMaxHeight自体はほぼ横ばい（7.55 vs 7.70）のまま**。
     v2レビューで原因を「単一支柱（brace無し）構造は揺れイベントで負荷が一律に跳ね上がるため、
     耐荷重を上げてもそのぶん高く登ってから同程度の相対圧で頭打ちになる」構造的特性と特定済み。
     score/collapseEvents/blocksLostという効率指標での改善を「鑑定投資の効果」の根拠として
     採用しているため対応必須ではないが、height自体も動かしたい場合はbrace無し構造の揺れ
     ダメージ特性自体の見直しが必要になる（大掛かりな変更のため4回目以降で要否を判断）
  3. **【軽微・継続】invalidActions平均がcareful37.5・braced35.8とv1（20前後）よりむしろ増加している**が、
     v1では死亡によりセッションが3600tick未満で終了していたのに対しv2は死亡が解消され常に3600tick
     丸ごと実行されるようになったため、単純にプレイ時間が伸びた分の比例増加である可能性が高い
     （発生率自体が悪化したかは未検証）。優先度は低いが余裕があれば原因を特定すること
  4. v2で対応済み: 鑑定投資ROI問題（品質ばらつき拡大0.5〜1.5、鑑定コスト1/3、`qualityWeightFactor`
     新設）、目標到達後の死亡スパイラル（地上HP回復、HPベースのボット退避行動、マイルストーン報酬の
     高度60超での逓減、生還スコアボーナス）、brace連携判定のO(blocks³)パフォーマンス問題
     （O(blocks×braces)へ最適化）。詳細はreviews/019-building-hallmark-v2.md参照
- **【最優先・28サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜019v2を含め直近28サイクル連続で未実施。サイクル25・2回目
  （本PR）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを
  起動できない（承認者不在のため）」という明文化された拒否を再度受け取った（28回目の再確認、
  `.claude/launch.json`にport 5188でhallmark設定を追加済み）。無人スケジュール実行が続く限り
  構造的に解消しないため、人間の対話セッションでこのリポジトリを扱う機会があれば最優先で実施すること**
- **P01/P02運用ルール**: 015-flagship-bulwark-final以来の「P01は夜フェーズ検証の対象外、コア機構の
  判定はP02の結果のみで行う」運用は、昼夜サイクルを持たないbuilding単体プロトタイプ（019）には
  そのままは適用されない。019ではcareful=P01代替・sloppy=P02代替として診断ボットを流用しており、
  真のP01/P02専用ロジックではない点をv3以降も明記すること

## 過去のサイクル25・2回目（完了・アーカイブ）

- サイクル: 25
- 要素: **建築（building）**。対象ゲーム番号: **019-building-hallmark**
- サイクル25・1回目（reviews/019-building-hallmark-v1.md）の重大バグ2件（鑑定投資ROIほぼゼロ、
  目標到達後の継続プレイでcareful/braced20/20シード全滅）に対応しFIX+REVIEWを完了した
  （詳細は上記「現在位置」参照）。修正過程で発覚したbrace連携判定のO(blocks³)パフォーマンス問題も
  O(blocks×braces)へ最適化し解消した

## 過去のサイクル25・1回目（完了・アーカイブ）

- サイクル: 25
- 要素: **建築（building）**。cycle24-final（reviews/018-flagship-veinsight-final.md）の提案(1)を
  受け、004（単体プロトタイプ、かなり早い段階）・010（戦闘×建築の組み合わせ）以来、mining
  （003→011→017の3回再検証）や戦闘（002→009の2回再検証）と比べて明確に検証回数が手薄で、
  flagship側のバリケード・前線拠点システムも012からほぼ無改修のまま据え置かれていたため回帰した。
  017/018で確立した「非対称の情報公開」「投資シナジー」パターンをbuilding側に横展開する新規単体
  プロトタイプ`019-building-hallmark`を実装し、v1レビューを完了した（詳細は上記「現在位置」参照）
