...
- **サイクル23・3回目（FIX only、本PR）からの引き継ぎ事項（優先度順）**:
  1. 【最優先・4回目で必ず検証】v2引き継ぎ#1（band5到達性の検証）に対応し、`headless/simulate.ts`に
     3つ目のbot戦略`pusher`を新規追加した（`src/core/game.ts`は無変更）。plannerと同じ購入優先度・
     レーン選択を使うが、帰還判断だけ`miningRiskLevel==='danger'`（安全マージン15込み）ではなく
     `estFuelToReturn`ちょうど＋バッファ0まで安全マージンを意図的に削って前進する。
     **判明した重要な事実**: バッファを0〜2にすると死亡0/20のままavgMaxDistanceが160.0(planner)→
     166.7(pusher)へ微増するのが安全に押し切れる限界で、バッファを少しでも負（-5等）にすると
     即座に死亡20/20へ崖のように転落する（中間の緩やかなリスク/リターン曲線が存在しない）。
     さらに**band5(x=201-240)はバッファ0でも166.7止まりで到達不能**——これは安全マージンの
     問題ではなく、燃料タンク最大Lv(6)の最大燃料340に対し「往路（掘削コスト込みで割高）＋
     復路（床のみで距離×1）」の物理的な燃料総量が構造的に足りないため、**往復前提である限り
     どれだけリスクを取ってもband5には届かない**という設計上の限界が判明した。4回目
     （FINAL REVIEW）でこの発見を踏まえ、(a)band5は「一方通行の捨て身プレイ」でのみ到達可能な
     区画として意図通りと判定するか、(b)燃料タンクのmaxLevel引き上げ・道中の燃料回収要素等の
     設計変更で往復可能にすべきか、両ペルソナ視点で最終判断すること
  2. v1課題#2（豊富さの高いレーンほど高難度鉱石が混じりやすい）・一本道フォーク分岐の構造的
     トレードオフ（v1課題#3）はv2で決着済み、追加対応は不要
  3. 3回目（本PR）はgame.ts本体の変更なし。`headless/simulate.ts`へのpusher戦略追加のみで
     build/simulateとも正常終了を確認済み
- **【最優先・20サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜017-v2を含め直近20サイクル連続で未実施。サイクル23・2回目でも
  `preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを起動できない
  （承認者不在のため）」という明文化された拒否を再度受け取った（20回目の再確認、`.claude/launch.json`
  にport 5186でforkshaft設定を追加済み）。無人スケジュール実行が続く限り構造的に解消しないため、
  人間の対話セッションでこのリポジトリを扱う機会があれば最優先で実施すること**
- **P01/P02運用ルールの見直し（サイクル23で採掘パートへ回帰したため）**: 013〜016の昼夜サイクル・
  拠点防衛系ゲームでは「P01は夜フェーズ検証の対象外、コア機構の判定はP02の結果のみで行う」を
  デフォルト運用としていたが（reviews/015-flagship-bulwark-final.md参照）、017は昼夜サイクルを
  持たない採掘単体ゲームのためこの制約は適用されない。017-v1ではP01の評価軸（A1経済・A3シナジー・
  A5仮説検証）を主たる判断材料として両ペルソナを対等に扱った（結果は上記の引き継ぎ事項を参照）
