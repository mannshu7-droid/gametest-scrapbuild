...
- 【軽微・継続】HUDの「帰還推定燃料」表示は3サイクル連続（002 v2〜final、003 v1〜final）でブラウザの
  スクリーンショット取得がタイムアウトしており実機目視未確認。リポジトリ共通の環境制約であり個別ゲームの
  問題ではない。次回以降も「コード実装済み・getState()の値は正しい」を目視確認の代替エビデンスとして扱う
- 003-mining-deepvein final（reviews/003-mining-deepvein-final.md）からサイクル3（建築）への提案:
  ...
  4) 経済ループの指標が目標未達に見えても、経済パラメータをいきなり調整せず、まず検証bot・実プレイの
     行動精度が指標に影響していないかを疑う
- 004-building-skyspire v1（reviews/004-building-skyspire-v1.md）からv2（次回）への引き継ぎ事項:
  1) 【致命・最優先】目標高度15でもcareful戦略（設計重視bot）が平均高度11で頭打ちになり6分間で未到達。
     単一支持経路（木構造）の負荷モデルでは土台を広げても荷重が分散されないことが原因。
     braceの肩代わり率をさらに強化する／複数支持経路へ物理モデルを変更する／目標高度をさらに下げる、
     のいずれかで「6分の実プレイで無理なく到達できる」水準に調整すること
  2) 【重大】1回の大崩落から経済的に回復できず、崩落後は残り時間のほぼ全てが停滞になる
     （ブラウザAIP実測でtick91崩落後、tick3600まで最高到達高度が一度も更新されず）。
     崩落時に資材の一部をその場で回収できる、または土台の実績を保持する等、
     「1ミスでセッションが壊れない」復帰導線を追加すること
  3) 揺れ予兆中でもmaxStressRatioが下がらない場面があり「待てば安全」が保証されない。
     揺れ予兆〜本震の間にプレイヤーが取れる有効な回避行動を明確にする
  4) P02（没入型）にとってE4（ロールプレイ余地）がほぼ対象外になる仕様上の弱点が判明。
     v2で無理に解消する必要はないが、最終レビューでの「本命ゲームに採用すべき部分」判断時に
     建築要素は数値ゲー寄りになりやすい点を踏まえること
