...
- **【最優先・17サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ...
  `preview_start`できる**
- **サイクル21-finalで確定した運用ルール（引き続き適用中）**: P01ペルソナは013・014・015・016の
  4ゲーム連続で夜フェーズ到達前に前線戦闘で決着し、昼夜サイクル系のコア機構を一度も検証できて
  いない。今後この系統（昼夜サイクル・拠点防衛が主軸）のゲームでは「P01は夜フェーズ検証の対象外、
  コア機構の判定はP02の結果のみで行う」ことをデフォルト運用とする（詳細は
  reviews/015-flagship-bulwark-final.md「捨てる・見直すべき部分」参照）
