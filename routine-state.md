...
- サイクル: 25
- 要素: **建築（building）**。対象ゲーム番号: **019-building-hallmark**（サイクル25・1回目で新規実装済み）
- 次に行う回: **2回目（FIX＋REVIEW）**
- **サイクル25・1回目（reviews/019-building-hallmark-v1.md）からの引き継ぎ事項（2回目で最優先対応）**:
  1. **【重大・最優先】鑑定投資（ロット品質の非対称公開＋選別配置）の単独効果がほぼゼロ**。
     braceを使わない`informed`戦略（鑑定Lv1・ロット選別配置）が同条件の`sloppy`戦略（鑑定なし・
     盲目配置）をavgMaxHeight・avgScoreいずれでも明確に上回れなかった（018 v1の「情報投資ROIマイナス」
     と同型の問題）。対応候補: 品質のばらつき幅（現状0.7〜1.3倍）の拡大、鑑定コストの引き下げ、
     または品質がstressRatio計算に与える影響（レバレッジ）の強化。018 v1→v2の対応（価格引き下げ＋
     ボット側の「情報を実際に活用する判断」強化）を参考にすること
  2. **【重大】目標到達後もセッションを継続する仕様変更（004最大の積み残し対応）により、
     brace連携・鑑定を使いこなす強い戦略ほど登り続けた結果、`careful`/`braced`戦略が
     20/20シード全てHP0で死亡する**。004の同等戦略（careful）は崩落0・無傷で完走していたのに
     対し真逆の結果。対応候補: 一定HP以下で自動的に安全域へ退避するインセンティブ、高度に応じた
     収穫逓減で無限に登り続けるメリットを薄める、またはスコア計算に「生還ボーナス」を設けて
     生きて地上に戻ることの価値を明示する等。「クリア後も遊べる構造を入れるときは、同時に
     安全に退場するインセンティブもセットで設計する必要がある」という今回のLearningsを踏まえること
  3. brace連携共鳴（新規システム2）は非常に強く機能した（brace無しavgMaxHeight7.5→brace連携込み
     55.9、+645%）。この部分は**v2で弱める必要はない**。#1・#2の対応時にbraceの数値自体
     （BRACE_RADIUS_LINKED/LINK_FACTOR）を不用意に変更しないよう注意すること
  4. invalidActions平均20前後（careful/braced戦略）が軽微に発生している。stabilizerを一切
     使わないボット設計が原因の可能性が高く、優先度は低いが余裕があれば原因を特定すること
- **【最優先・27サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜019v1を含め直近27サイクル連続で未実施。サイクル25・1回目
  （本PR）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを
  起動できない（承認者不在のため）」という明文化された拒否を再度受け取った（27回目の再確認、
...
- **P01/P02運用ルール**: 015-flagship-bulwark-final以来の「P01は夜フェーズ検証の対象外、コア機構の
  ...
  真のP01/P02専用ロジックではない点をv2以降も明記すること
