...
- 要素: **建築（building）**。対象ゲーム番号: **019-building-hallmark**（サイクル25・1回目で新規実装、
  2回目でFIX+REVIEW完了、3回目でFIX only完了）
- 次に行う回: **4回目（FINAL REVIEW）**
- **サイクル25・3回目（本PR、修正内容は下記実行履歴表参照）からの引き継ぎ事項（4回目で総括すること）**:
  1. **【軽微・解消】careful<braced逆転（バグ#5）は3回目で対応済み**: `lotIndex`明示指定の設置に
     少額スコアボーナス（`INFORMED_PLACEMENT_SCORE_BONUS=1.5`）を追加し、20シードでcareful
     2290.60 > braced 2201.15（+4.1%）に回復。詳細は下記実行履歴表の該当行参照
  2. **【軽微・未対応・持ち越し】informedのavgMaxHeightが依然として頭打ち**（単一支柱構造は
     揺れイベントで負荷が一律に跳ね上がるため、耐荷重を上げてもそのぶん高く登ってから同程度の
     相対圧で頭打ちになる構造的特性、v2レビューで特定済み）。大掛かりな設計変更（揺れダメージ特性
     自体の見直し）を要するため、4回目のFINAL REVIEWで「本命ゲームに採用すべき部分・次に検証すべき
     こと」として総括し、必要なら次サイクル以降の新規仕様書に持ち越すこと
  3. **【軽微・解消】invalidActions平均増加（v1約20→v2約36）は3回目で調査済み**:
     鑑定の有無（careful/braced）に関わらず同程度（10シードで35〜51件）であることを確認し、
     死亡解消でセッションが常に3600tick丸ごと実行されるようになった分の比例増加と結論、
     対応不要と判断（コード変更なし）
...
- **P01/P02運用ルール**: 015-flagship-bulwark-final以来の「P01は夜フェーズ検証の対象外、コア機構の
  ...
  真のP01/P02専用ロジックではない点を4回目でも明記すること

## 過去のサイクル25・3回目（完了・アーカイブ）

- サイクル: 25
- 要素: **建築（building）**。対象ゲーム番号: **019-building-hallmark**
- v2からの持ち越し事項3件のうちバグ#5（careful<braced逆転）に対応し解消、バグ#3（invalidActions増加）
  を調査し対応不要と結論、バグ#2（informedのheight頭打ち）は大掛かりな変更を要するため4回目へ持ち越した
  （詳細は下記「現在位置」および実行履歴表参照）
