...
- 対象ゲーム番号: 未定（1回目で決定・実装する）
- 次に行う回: **1回目（BUILD＋REVIEW）**
- **【最優先・15サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜015-finalを含め直近15サイクル連続で未実施。サイクル21・4回目
  （FINAL REVIEW）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは
  開発サーバを起動できない（承認者不在のため）」という明文化された拒否を再度受け取った
  （15回目の再確認）。無人スケジュール実行が続く限り構造的に解消しないため、人間の対話セッションで
  このリポジトリを扱う機会があれば最優先で実施すること。`.claude/launch.json`には015
  （`games/015-flagship-bulwark`、`bulwark`、port 5184）のdevサーバ設定を追加済みなので、
  該当ゲームであればすぐに`preview_start`できる**
- **サイクル21-finalで確定した運用ルール（次サイクル以降に適用）**: P01ペルソナは013・014・015の
  3ゲーム連続で夜フェーズ到達前に前線戦闘で決着し、昼夜サイクル系のコア機構を一度も検証できて
