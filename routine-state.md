...
- サイクル: 17
- 要素: **未定（次回1回目のBUILD+REVIEWで決定）**。cycle16-final
  （reviews/010-combat-building-bastion-final.md）が、008・009・010の3ゲームを通じて「戦闘」
  （002/008/009で3回検証済み）と「建築」（004/006/007/008/010で5回検証済み）の知見はほぼ出尽くした
  一方、**「採掘」要素は003（サイクル2）以来009・010の2サイクル連続で意図的に除外されており、
  009・010で確立した横方向帯状フィールド構造での採掘由来パターンの移植性検証（戦闘・建築の
  「本物の建築は代替が効かない」検証と対になる、要素ローテーションの最後のピース）が未実施**と
  結論づけ、次サイクルは**採掘単体の横展開検証（`011-mining-<名前>`、優先案）**または
  **本命ゲームへの統合設計フェーズへの着手（次点案、ただし採掘の横展開を先に済ませる方が手戻りが
  少ないと付記）**を提案した
- 対象ゲーム番号: 未定（次回1回目で決定。cycle16-finalの優先案に従うなら`011-mining-*`）
