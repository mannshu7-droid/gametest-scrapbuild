...
- **サイクル23・1回目（reviews/017-mining-forkshaft-v1.md）からの引き継ぎ事項（優先度順）**:
  1. 【中・新規、次回優先】共鳴チャージの発動頻度が低い（20シード平均resonanceTriggers 0.6回）。
     「ドリル強化とビルドのシナジー」というサイクル23の検証テーマの核心であるにもかかわらず、
     擬似実プレイ（P01/P02とも）でも狙って使うシナジー体験としての印象が薄かった。上限チャージの
     初期値引き下げ・鉱石ごとのチャージ増分見直し等で発動頻度を引き上げることを検討する
  2. 【参考、対応要否は次回で判断】「豊富さの高いレーンほど高難度鉱石（要求ドリル威力の高いタイル）も
     混じりやすい」という現在の乱数設計が、豊富さを盲目的に追う戦略にとってN6（渋い報酬）寄りの
     徒労感を生むリスクがある。P01視点での擬似実プレイで観測されたが致命ではないため、v2で
     複数シードの追加検証を行った上で対応要否を判断すること
  3. 一本道フォーク分岐は、003/011の「複数レーンへ即座に迂回できる」設計と比べ、単一の要求ドリル
     威力ゲートが致命傷になりやすい構造的トレードオフを持つ（v1では全滅パターン0/20で実害なし）。
     バランス調整は不要だが、次に類似の一本道分岐を設計する際の留意点として記録した
- **【最優先・19サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜017-v1を含め直近19サイクル連続で未実施。サイクル23・1回目でも
  `preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを起動できない
  （承認者不在のため）」という明文化された拒否を再度受け取った（19回目の再確認、`.claude/launch.json`
