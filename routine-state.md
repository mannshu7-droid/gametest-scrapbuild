...
- 要素: **建築（building）**。cycle24-final（reviews/018-flagship-veinsight-final.md）の提案(1)を
  ...
  flagship側のバリケード・前線拠点システムも012からほぼ無改修のまま据え置かれていたため回帰する。
  017/018で確立した「非対称の情報公開」「投資シナジー」パターンをbuilding側に横展開できるかを
  検証する新規単体プロトタイプとする（cycle24-finalの提案(1)、次点(2)のflagship直接統合は見送り、
  まず単体で新規メカニクスを検証してから統合する017→018の前例を踏襲する）
- 対象ゲーム番号: 未定（次回1回目で決定）
- 次に行う回: **1回目（BUILD＋REVIEW）**
- **サイクル24-final（reviews/018-flagship-veinsight-final.md）からの引き継ぎ事項（継続分）**:
  1. 018で「非対称の情報公開」「共鳴チャージ」をmining側へ統合しコアファン仮説の実証に成功した
     （'planner'が'blind'をavgScore+16.3%・avgMaxDistance+18.9%・avgOreMined+151.6%で上回る）。
     「新規投資カテゴリは単体プロトタイプの価格をそのまま移植せず、既存カテゴリと同格ではなく
     明確に安い価格帯（今回は-39%〜-54%）に置く必要がある」という教訓は今後の新規カテゴリ追加
     （building側の鑑定投資等）でも指針とすること
  2. P01（野望型）は013〜018の6世代連続で夜フェーズ・バンド境界到達前に早期死亡し、昼フェーズ
     限定の新機構（今回はA5仮説検証・A1経済）の体感評価ができない構造的ミスマッチが続いている。
     「P01は夜フェーズ検証の対象外」という運用ルールは維持しつつ、評価したい軸が昼フェーズ限定の
     場合は専用の代替ボット設計（夜間交戦を避け昼の経済活動を優先する等）を今後検討する余地がある
  3. P02視点のE4（想像の余地・ロールプレイ）を「mining×拠点防衛の統合が補えるか」直接評価
     できなかった（ヘッドレスbot指標では実証不能）。building側の新規プロトタイプでも同様の
     制約があることを前提に、ブラウザ実プレイが実現するまでE4は「未確定」のまま扱うこと
- **【最優先・26サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜018finalを含め直近26サイクル連続で未実施。サイクル24・4回目
  （本PR）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを
  起動できない（承認者不在のため）」という明文化された拒否を再度受け取った（26回目の再確認、
  `.claude/launch.json`にport 5187でveinsight設定を追加済み）。無人スケジュール実行が続く限り
  構造的に解消しないため、人間の対話セッションでこのリポジトリを扱う機会があれば最優先で実施すること**
- **P01/P02運用ルール**: 015-flagship-bulwark-final以来の「P01は夜フェーズ検証の対象外、コア機構の
  判定はP02の結果のみで行う」デフォルト運用を継続する。新要素の評価軸（経済・シナジー・仮説検証）が
  P01の評価軸に該当する場合は、上記引き継ぎ事項#2の通り評価手法の改善余地を検討すること
