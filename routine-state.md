...
- サイクル: 29
- 要素: **建築（building）単体**。022-final（reviews/022-flagship-hearthline-final.md）の提案(2)、
  019-finalから持ち越されていた「多脚・分岐構造の建築単体プロトタイプ」を採用する。012（統合）→013〜016
  （防衛系統合の深掘り）→017（採掘回帰）→018（統合）→019（建築回帰）→020（統合）→021（戦闘回帰）→
  022（統合）という「統合フェーズと単体要素検証の往復」パターンに従い、022の統合完走を受けて単体要素へ
  回帰する。019-finalが積み残した「単一支柱構造の物理的限界（informedのavgMaxHeight頭打ち）」を、
  プレイヤーが経路（脚・分岐）を選べる多脚・分岐構造で解消できるかを検証する。022-finalで得た知見
...
- サイクル28・4回目（FINAL REVIEW、reviews/022-flagship-hearthline-final.md）で最終総括し判定**FIX**
  （P01約4.1/5・P02約4.0/5、020-finalの約3.9/3.7から改善）。標本を40シード標準・10シード長時間（60000tick）
  へ倍増して3回目の結論（mason/patrollerの優劣「逆転」は誤記であり、標準・長時間ともmasonが一貫して安全）の
  頑健性を再確認し、コアファン仮説（投資あり10〜15%死亡 vs 投資なし57〜60%死亡、死因は前者が100%
  night-field・後者はday-siege主体）が崩れないことを確認した。新規知見として、タレットのクラスタ配置という
  受動的投資だけで効果の大部分（60%→12.5%）が出ており、鑑定+ダッシュ反応という能動的な上乗せ行動の追加
  ゲインは比較的小さい（12.5%→10.0%）ことを発見し、v1 Learnings「投資が見えることと行動することは全く別」を
  より精密な構図へ修正した。パトロール圏・鑑定precisionの分離設計は本命ゲームへの採用を推奨、
  recommendedReturnLane（レーン誘導）は本命統合時にプレイヤー選択式の一方の選択肢とするか寄り道の時間
  コストを相殺する仕組みを検討すべきと結論。次サイクルは019-finalから持ち越しの「多脚・分岐構造の建築単体
  プロトタイプ」（023-building-*）を提案し、routine-state.mdをサイクル29・run1へ進めた
- 3回目の申し送り（対応済み）: mason（ダッシュ反応、即応的）とpatroller（レーン誘導、
...
- サイクル27（021-combat-duskrun）の結論: 判定FIX。コアファン仮説の前半（帰還判断のスキル性）はpush-forever
  ...
  購入順の機会費用を是正してno-decoy比-4.0%→-2.0%まで改善したが軽微な負のROIが残った。副産物として
...
- サイクル: 27
- 要素: **戦闘（combat）単体: 「日没の撤退戦」**。cycle26-final（reviews/020-flagship-linkmark-final.md）の提案(1)を採用した。
  ...
  「拠点から離れた場所で日没を迎え、追ってくるレイダーから逃げ切る／退きながら戦う」ことを主題にした新規プロトタイプを作った。
- 対象ゲーム番号: **021-combat-duskrun**（specs/021-combat-duskrun/spec.md、games/021-combat-duskrun/）
- サイクル27・4回目（FINAL REVIEW、reviews/021-combat-duskrun-final.md）で最終総括し判定**FIX**（P01約3.8/5・P02約3.2/5）。
  120シード×10戦略（v2からコード無変更）で再検証し、コアファン仮説の前半（帰還判断のスキル性）が4回通じて一貫して成立、
  後半（退路投資の悩ましさ）もdaylight・ルート選択の2系統で確立・囮も軽微な水準（-2.0%）まで改善したことを確認した。
  `returnMargin`計算式が正確すぎるため`nightEntries`は全戦略で0のまま残り「夜のフィールド死」の再現は未達（代わりに
  「日中の退却判断ミスによる包囲死」が主要な危険構造になった）ことを特定。次サイクルは020-flagship-linkmarkへ021の
  帰還マージン・daylight投資・ルート選択を統合する`022-flagship`を提案
- サイクル27・3回目（FIX only、レビューは書かずPR本文に記載）の要点: v2から持ち越した「囮(decoy)のROI」の
  購入順依存性を検証した。`headless/simulate.ts`の`decoy-heavy`戦略の購入順を`['maxFlare','maxFlare','atk','maxHp',...]`
  から`['atk','maxHp','maxFlare','maxFlare',...]`（atk/maxHpを先に確保してから囮投資へ進む）へ変更しただけで、
  avgScoreがno-decoy比-4.0%→**-2.0%**（3161.5→3228.1）、deathsが13/120→**4/120**（adaptive-route/no-decoyと同水準）
  に改善した。これにより「囮メカニクス自体（DECOY_STUN_TICKS/DECOY_RADIUS等）は問題なく機能しており、
  v1〜v2で観測された大きなROI逆転は主にテスト戦略側の不自然な購入順（機会費用）が支配的要因だった」ことを確定させた。
  ゲーム本体（src/core/game.ts）は無変更、headless/simulate.tsの`decoy-heavy`購入順のみ修正。
  120シード×10戦略で決定論・ソフトロック0件を再確認
...
- サイクル27・1回目は新規実装＋v1レビュー完了・判定FIX（reviews/021-combat-duskrun-v1.md）
