...
- サイクル: 18
- 要素: **本命ゲーム統合フェーズ（新規番号`012-flagship-*`）**。cycle17-final
  （reviews/011-mining-driftline-final.md）が、009（戦闘）・010（戦闘×建築）・011（採掘）の
  3サイクルを通じて横方向帯状フィールド構造上で戦闘・採掘・建築すべての単体パターンが
  機能することが出揃ったと結論づけ、010-final時点で条件付きだった「採掘の横展開を先に済ませてから
  統合フェーズへ着手する」が満たされたとして、戦闘・採掘・建築を同時に統合した本命ゲーム試作機
  （008とは別に、横方向帯状フィールド構造を土台にした「第二世代」の位置づけ）への着手を提案した。
  次点案として、3要素同時統合が複雑すぎる場合は先に未検証の2要素ペア（採掘×戦闘 or 採掘×建築）で
  切り分けることも提案されている（1回目の判断で採否を決めること）
- 対象ゲーム番号: **未定（1回目で`012-flagship-<名前>`または`012-mining-<combat/building>-<名前>`を
  決定）**
- 次に行う回: **1回目（BUILD＋REVIEW）**
- 引き継ぎ事項（cycle17-finalより）:
  1. 3要素同時統合はCLAUDE.mdの「1ゲーム=1要素」原則から意図的に外れる組み合わせ試作となる。
     routine.mdの要素ローテーション表「4以降は組み合わせ試作も選択可」の規定に従うため、
     仕様書冒頭でこの逸脱を明記すること（005〜010の組み合わせ試作と同じ扱い）
  2. 土台とすべき確立済みパターン: 建築=前線拠点（008パターン#7、010で最重要と確定）、
     採掘=常設ショップ・安全マージン公開・地形由来の危険（003・011で実証）、
     戦闘=EVベースのヒント設計（009で提案された12番目の共通パターン候補、まだ採用されていない）、
     共通=「詰みからの脱出手段」（money版・HP版に続き、統合時に新たに詰みが見つかったリソースが
     あれば同型で追加する）
  3. 危険度UIヒント（`combatRiskLevel`/`miningRiskLevel`相当）の評価は探索精度に強く依存すると
     011-finalで再確認された。本命ゲームのAI評価では複数の探索精度のプレイヤー/botを比較する
     手法を標準化すること
  4. 5サイクル連続でブラウザAIP実プレイが未実施（無人スケジュール実行のため）。人間の対話
