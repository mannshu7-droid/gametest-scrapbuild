...
- サイクル15・2回目（FIX+REVIEW、reviews/009-combat-ironmarch-v2.md）で対応した事項・残課題:
  1. 【解消】v1バグ#2（保護範囲外ソフトロック）: `nearestWaystationDistance`にx=0（フィールド
     左端の出撃地点）を保護基準点として追加し、他ウェイステーションと同じ半径3マス保護・回復を
     与えた。seed301（v1で12000tick停滞・over=false）で再検証し、1523tickで400マス完走・勝利に
     転換したことを直接確認した
  2. 【解消】v1バグ#3（危険ルートの魅力不足）: 撃破報酬倍率×1.5→×1.8、ウェイステーション到達
     ボーナスを危険セグメント踏破後+25（安全時は+10のまま）に分離。20シード比較で
     balanced-riskyがbalanced-safeを逆転（avgScore1916.5 vs 1448.9、v1は426 vs 1158で逆だった）
  3. 【部分改善・残課題】v1バグ#4（危険度ヒントの感度不足）: 危険セグメントの推奨HP倍率
     ×1.2→×1.35に引き上げ、atk特化ビルドでは実際に'danger'判定が発生するようになった一方、
     balanced戦略は投資が分散的でバランスよく育つため成長ペースが要求水準の上昇にほぼ追随し、
     balanced-adaptiveがbalanced-riskyと常に同一判断になる問題自体は解消しきれなかった。
     根本対応には「生存可否だけでなくEV（どちらが得か）を直接計算して見せる」設計変更が必要
     （reviews/009-…-v2.md Learnings参照）。3回目（FIX only）または以降で判断すること
  4. ブラウザAIP実プレイは本セッションも無人のスケジュール実行のため`preview_start`が
     「無人セッションからは起動できない」旨で拒否された（2回連続）。代わりに`window.__AIP__`と
...
- サイクル14・1回目（BUILD+REVIEW相当）は、cycle13-finalのLearningsが提案した2つの着眼点
  ...
  対象外になっておらず単一カテゴリ隔離が不完全」というバグ（cycle11導入時から3サイクル
