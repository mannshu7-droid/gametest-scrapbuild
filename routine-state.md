...
- サイクル: 24
- 要素: **本命ゲーム統合フェーズ・第七世代**（`018-flagship-*`、mining側への017パターン統合）。
  ...
  シナジー」を本命ゲームのmining側に統合することを決定した（サイクル24・1回目で対象ゲーム番号と
  詳細仕様を確定すること）
- 対象ゲーム番号: 未定（サイクル24・1回目のBUILDで`018-flagship-<名前>`として決定・実装する）
- 次に行う回: **1回目（BUILD+REVIEW）**
- **サイクル23-final（reviews/017-mining-forkshaft-final.md）からの引き継ぎ事項（優先度順）**:
  1. 017の最終決定: band5（x=201-240）は燃料タンク最大強化・安全マージンを0まで削っても往復前提では
     構造的に到達不能と判明し、**バグではなく「一方通行の捨て身プレイ」専用区画として受け入れる**と
     判定した（spec.md「band5到達性についての最終決定」節参照）。ただし「安全圏の外側にボーナス
     領域を置く場合、マージンを0まで削っても+4%程度しか進めず僅かでも踏み込むと死亡率が0%→100%へ
     崖状に転落する」非連続なリスクカーブはP01視点でN6（渋い報酬）に近いと評価された。018や
     今後の新規ゲームで同種の「安全圏外ボーナス領域」を設計する際は、崖状ではなく連続的な
     リスク/リターン曲線（中間的なリスクにも段階的な追加報酬）を検討すること
  2. mining単体プロトタイプ（003・011・017）はP02の最重要軸E4（想像の余地・ロールプレイ）を
     構造的に満たせないことが3例で一貫して確認された。018は統合フェーズのため前線拠点・拠点防衛の
     「生活基盤を守る」文脈がE4に部分的に応える可能性がある——018のレビューでは改めてE4評価を
     行い、mining×他要素の統合がE4を補えるかを確認すること
  3. 次点候補（018が複雑化しすぎる場合の代替案）: 017が発見した崖状リスクカーブを主題に据え、
     中間的なリスクテイクに応じた段階的な追加報酬（部分的な燃料回収ポイント等）を持つ新規mining
     単体プロトタイプ
  4. 017のゲーム本体（`src/core/game.ts`）はv1完成時点から一度も変更されていない安定版。
     `games/017-mining-forkshaft/`は今後の参照実装として保持する
- **【最優先・22サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜017-finalを含め直近22サイクル連続で未実施。サイクル23・4回目
  （本PR）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを
  起動できない（承認者不在のため）」という明文化された拒否を再度受け取った（22回目の再確認、
  `.claude/launch.json`にport 5186でforkshaft設定を追加済み）。無人スケジュール実行が続く限り
  構造的に解消しないため、人間の対話セッションでこのリポジトリを扱う機会があれば最優先で実施すること**
- **P01/P02運用ルール**: 018は昼夜サイクル・拠点防衛を含む統合フェーズへ復帰するため、
  013〜016で確立した「P01は夜フェーズ検証の対象外、コア機構の判定はP02の結果のみで行う」
  デフォルト運用（reviews/015-flagship-bulwark-final.md参照）を再適用すること。ただしmining側の
  新要素（非対称情報公開・共鳴チャージ）はP01の評価軸（A1経済・A3シナジー・A5仮説検証）が
  主たる判断材料になるため、昼フェーズの採掘パートに限りP01の評価も引き続き重視すること
