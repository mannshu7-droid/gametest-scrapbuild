...
- **サイクル24・1回目（reviews/018-flagship-veinsight-v1.md）の判定はFIX。2回目で対応すべき課題
  （優先度順）**:
  1. **重大**: コアファン仮説の中核「情報投資（探査・共鳴チャージ）が経済成長の差になる」が
     v1時点では不成立。scanner/chargeを一切買わない検証bot'blind'が、両方を最優先で買う'planner'を
     avgScore（20シード平均2234.5 vs 1972.5）・avgMaxDistance（104.3 vs 98.0）・avgKills（213.9 vs
     185.9）のいずれでも上回った。scanner(基礎コスト18)/charge(22)がdrill(30)と同格の価格帯で
     drill/fuel投資と競合する機会費用が、フォグ回避・共鳴ボーナスのリターンを上回っていることが
     原因と推定される。次回はscanner/chargeの基礎コスト引き下げ、または共鳴チャージの増分・上限
     拡大（017 v2の対応を参考に）を検討し、修正後に'planner'が'blind'を上回ることを確認すること
  2. **中（検証ボット側の限界、ゲーム本体のバグではない）**: `headless/simulate.ts`の`canDig()`が
     フォグ越しの`UNSCANNED`タイルの要求ドリル威力を実際より低く見積もり（`TIER`未定義のためtier0
     扱い）、掘削不能なタイルへの無為な試行を止められない。'blind'が'planner'を上回った結果を
     歪めている懸念があるため、比較の信頼性を上げるには`UNSCANNED`を「未知＝最悪ケースを仮定」する
     判定へ改善する必要がある（バグ#1の対応と合わせて検証し直すこと）
  3. 軽微: 共鳴チャージの発動頻度が017 v1と同様に低め（cautious/pusherで平均7〜13回/セッション）。
     ヘッドレスbotに「探査済みの豊富なレーンへ計画的に進む」戦略（016のタレット配置v3で追加した
     `pickDefensiveTurretLane`と同種の改善）を追加し、真の評価を行うことも検討する
  4. P01のための「探査結果を見てレーンを選ぶ」計画的なbot戦略が未整備。A5（仮説検証）・A1（経済）の
     評価がボット側の限界で歪んでいる懸念があり、次回以降で追加を検討する
- **サイクル23-final（reviews/017-mining-forkshaft-final.md）からの引き継ぎ事項（継続分）**:
  1. 017の最終決定: band5（x=201-240）は燃料タンク最大強化・安全マージンを0まで削っても往復前提では
     ...
     判定した（specs/017-mining-forkshaft/spec.md「band5到達性についての最終決定」節参照）。今後
     同種の「安全圏外ボーナス領域」を設計する際は、崖状ではなく連続的なリスク/リターン曲線
     （中間的なリスクにも段階的な追加報酬）を検討すること
  2. mining単体プロトタイプ（003・011・017）はP02の最重要軸E4（想像の余地・ロールプレイ）を
     構造的に満たせないことが3例で一貫して確認された。018のレビューでは改めてE4評価を行い、
     mining×他要素の統合がE4を補えるかを確認すること（v1ではヘッドレス指標のみで直接評価できず、
     ブラウザ実プレイが実現すれば新たな評価材料になるとv1で記録した）
  3. 017のゲーム本体（`src/core/game.ts`）はv1完成時点から一度も変更されていない安定版。
     `games/017-mining-forkshaft/`は今後の参照実装として保持する
- **【最優先・24サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜018v1を含め直近24サイクル連続で未実施。サイクル24・1回目
  （本PR）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは開発サーバを
  起動できない（承認者不在のため）」という明文化された拒否を再度受け取った（24回目の再確認、
  `.claude/launch.json`にport 5187でveinsight設定を追加済み）。無人スケジュール実行が続く限り
  構造的に解消しないため、人間の対話セッションでこのリポジトリを扱う機会があれば最優先で実施すること**
- **P01/P02運用ルール**: 018は昼夜サイクル・拠点防衛を含む統合フェーズのため、013〜016で確立した
  「P01は夜フェーズ検証の対象外、コア機構の判定はP02の結果のみで行う」デフォルト運用
  （reviews/015-flagship-bulwark-final.md参照）を継続する。ただしmining側の新要素（非対称情報公開・
  共鳴チャージ）はP01の評価軸（A1経済・A3シナジー・A5仮説検証）が主たる判断材料になるため、
  昼フェーズの採掘パートに限りP01の評価も引き続き重視すること
