...
- サイクル19・1回目（BUILD＋REVIEW、reviews/013-flagship-nightwatch-v1.md）で判明した事項:
  1. 昼夜フェーズ遷移・レイダーの一括スポーン（既に掘削済みの全FLOORタイルからband依存の重み付けで
     選出）・拠点HPの増減・新規危険度ヒント`raidRiskLevel`はいずれもクラッシュなく動作し、
     P02の擬似実プレイでホームHPが夜間に実際に減少（400→390）することを確認した。コアファン
     仮説の前半（掘った道が夜には拠点への侵攻路になる）は機能している
  2. **10シード×2戦略のヘッドレスシミュレーションで両戦略とも生存率0%**、cautiousは全10シードが
     新規敗北条件「ホーム破壊(homeDestroyed)」で終了。原因は3件のバグに特定した:
     致命#1「夜間レイダーが昼になっても消滅・撤退せず複数夜にまたがり拠点を削り続ける」、
     重大#2「拠点内ではショップ購入が迎撃より常に優先され、隣接脅威があっても迎撃されない」、
     重大#3「cautious戦略はバリケード建設条件（交戦距離3以上）に交戦距離2の設定では到達できず
     構造的に一度もバリケードを建てない」（012では拠点が無条件で安全だったため問題化しなかった
     設計前提が、013でレイダーの拠点侵入を許可したことで崩れた）
  3. コアループ・昼夜遷移に構造的欠陥はなく、原因を特定できた修正可能なバランス/ロジックバグと
     判断して判定**FIX**。次回（2回目）はバグ#1〜#3の修正に優先度順で着手する
  4. 本セッションも無人のスケジュール実行のため`preview_start`は試みず、P01/P02のボット設定
     （優先度・交戦距離・撤退HP閾値、加えて夜間帰還ロジック）でcore `Game`クラス直接step()による
     擬似実プレイを実施した。一時検証スクリプトはレビュー後に削除済み。7サイクル連続で未実施の
