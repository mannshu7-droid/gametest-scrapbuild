...
- 要素: **本命ゲーム統合フェーズ（flagship）: 建築パートへの非対称品質公開＋隣接連携共鳴の統合**。
  ...
  018-flagship-veinsight（拠点防衛三本立て＋mining側の非対称情報公開・共鳴チャージ）をベースに、
  019で確立したbuilding側の2パターン（**建材ロットの非対称品質公開＝鑑定`appraisal`投資**・
...
- 対象ゲーム番号: **020-flagship-linkmark**（サイクル26・1回目で新規実装＋v1レビュー完了・判定FIX、
  2回目でFIX+REVIEW完了・v2レビュー判定FIX、3回目でFIX only完了）
...
- サイクル26・3回目（FIX only、詳細はPR本文）の要点: v2の3件を調査・対応した。**調査で判明した構造的事実**:
  (1) 死亡はほぼすべて「夜にフィールド（拠点圏外）でレイダーに殺される」もので、拠点内の死亡は22件中1件
  （wall 22件中1、mason 27件中2）。拠点のタレット品質・連携は「フィールドの採掘者の生存」に届かない
  (2) 自宅HP400・昼の回復1.5×1200tickで、1夜15〜65の拠点被害が翌朝に必ず全快しており、**拠点被害（BDT）が
  生存にもscoreにも一切効かなかった**（v2で見えた「クラスタ=BDT小」は結果に繋がらない偽のトレードオフで、
  45000tickでは分散rangerがscore4796 vs wall4038・死亡22 vs 31/40と一方的に優位だった）
  (3) mason vs wallの死亡差は**ノイズ**: 品質を1.0に中立化して比べると120シード×2組でmasonの死亡が
  18/120（seed101-220）と28/120（221-340）に振れ、実ビルド27/22と符号も逆転（鑑定の購入順位を
  basedefense後/offense後へ変えても不変）。タレット/バリケードの品質が死亡を増やすという証拠はない。
  **対応**: `HOME_BASE_MAX_HP` 400→250、`BASE_DAY_REGEN` 1.5→0.05へ変更し、後半（15夜以降）の被害が蓄積して
  自宅が落ち得るようにした（20000tick・30000tickの標準セッションはcautious/wall/mason/ranger 120シードと
  p02/p01n 20シードとも旧設定と一致＝無回帰、45000tickでranger homeDestroyed 5/40 vs wall 1/40・mason 0/40と
  クラスタ=堅守／分散=高スコアだが自宅を落とす、という長時間限定のトレードオフが出現）。タレット火力の過剰化（#3）は
  45000tickでhomeDestroyed 0・BDT非拘束のため過剰とは言えない（**ranger+盾が長時間では一方的に強い点のみ課題**）。
...
- 次に行う回: **4回目（FINAL REVIEW）**。p01n・p02の2ペルソナ擬似実プレイ（ブラウザAIP不可）で
  ...
  守らない構造（次サイクルで拠点防衛と採掘者の生存を繋ぐ設計＝夜にフィールドで死なない手段、を検討する価値）
  ③ranger+盾の長時間での一方的優位 ④ブラウザAIP実プレイ未実施の継続
...
- **【最優先・32サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜020v2を含め直近32サイクル連続で未実施。サイクル26・2回目でも
...
- サイクル: 25
- 要素: **建築（building）**。cycle24-final（reviews/018-flagship-veinsight-final.md）の提案(1)を受け、
  004（単体プロトタイプ、初期）・010（戦闘×建築の組み合わせ）以来検証回数が手薄だった建築パートへ
  回帰し、017/018で確立した「非対称の情報公開」（鑑定投資でロット品質を可視化）と「投資シナジー」
  （brace連携共鳴）をbuilding要素に横展開する新規単体プロトタイプ`019-building-hallmark`を実装した。
  004最大の積み残し（目標到達が速すぎてセッションが早期終了し金が余る）にも対応し、目標到達後も
  セッションを継続する仕様に変更した
- 対象ゲーム番号: **019-building-hallmark**（サイクル25・1回目で新規実装、2回目でFIX+REVIEW、
  3回目でFIX only、4回目でFINAL REVIEW完了）
- サイクル25・4回目（reviews/019-building-hallmark-final.md）で最終総括し判定**FIX**。20シード×5戦略
  （maxTicks=4000）・P01/P02擬似実プレイ（careful/sloppy代替、4シード×maxTicks=3600）の再検証で
  v3時点からのコード無変更・数値完全一致を確認。careful>braced逆転解消（+4.1%）と死亡0/20を
  v2・v3・finalの3回連続で維持し、informedのavgMaxHeight頭打ち（バグ#2）は単一支柱構造の物理的
  制約による「バグではなく構造的限界」と最終判定した（次サイクルの新規仕様書で構造自体の見直しに
  持ち越し）。P01基本点約4.1/5・P02基本点約3.3/5（P02はE4想像の余地の構造的欠如が主因、
  mining単体003・011・017と同型）。「補強材の隣接連携共鳴」を017/018に続く3例目の投資シナジー
  パターンとして、「安全地帯に実利＋HPベース自動退避」を「クリア後も遊べる」設計の完成テンプレート
  として採用。次サイクルは019の2パターンを本命ゲーム（flagship）の建築パートへ統合することを
  提案。ブラウザAIP実プレイは29サイクル連続で未実施
- サイクル25・3回目（FIX only、レビューなし・詳細はPR #100本文参照）: v2引き継ぎ事項3件のうち
  バグ#5（careful<braced逆転）に対応し解消（`lotIndex`明示指定の設置に少額スコアボーナス
  `INFORMED_PLACEMENT_SCORE_BONUS=1.5`を追加、20シードでcareful2290.60 > braced2201.15、+4.1%に
  回復）、バグ#3（invalidActions増加）を調査し鑑定の有無に関わらず同程度（35〜51件）で死亡解消に
  伴う比例増加と結論し対応不要、バグ#2（informedのheight頭打ち）は大掛かりな変更を要するため
  4回目へ持ち越した
- サイクル25・2回目（reviews/019-building-hallmark-v2.md）でv1の重大バグ2件（鑑定投資ROIほぼゼロ、
  目標到達後の継続プレイでcareful/braced20/20シード全滅）に対応しFIX+REVIEWを完了、判定**FIX**。
  品質ばらつき拡大・鑑定コスト引き下げ・`qualityWeightFactor`新設でinformedがsloppyをavgScore
  +11.8%・avgCollapseEvents-20%で上回るよう改善、地上HP自動回復＋HPベース退避行動＋マイルストーン
  報酬半減＋生還ボーナスでcareful/bracedの死亡を20/20→0/20に解消。修正過程で発覚したbrace連携判定の
  O(blocks³)パフォーマンス問題もO(blocks×braces)へ最適化し解消した。新規発生のcareful<braced逆転
  （-4.5%）はv3へ持ち越した
- サイクル25・1回目（reviews/019-building-hallmark-v1.md）で新規実装し判定**FIX**。ヘッドレス検証
  （20シード×5戦略）でbrace連携単独は劇的に機能（+645%）した一方、鑑定投資の単独効果はほぼゼロ
  （018v1と同型のROI不成立）と判明。さらに目標到達後も登り続ける強い戦略（careful/braced）が
  20/20シード全滅（HP0）する新規の重大課題も発見しv2へ持ち越した

## 過去のサイクル24（完了・アーカイブ）

- サイクル: 24
- 要素: **本命ゲーム統合フェーズ・第七世代**（`018-flagship-veinsight`、mining側への017パターン統合）。
  cycle23-final（reviews/017-mining-forkshaft-final.md）の提案(1)を受け、012（第二世代、戦闘×採掘×
  建築の初回統合）以来013〜016の4世代は昼夜サイクル防衛（脅威予告→恒久投資→配置建築）の掘り下げに
  専念しmining側のシステムは012時点からほぼ据え置きだったため、017で確立した「非対称の情報公開
  （探査Lv投資で採掘の豊富さが見える）」「共鳴チャージによる探査×ドリル威力×チャージの三位一体
  シナジー」を本命ゲームのmining側に統合した（017の「フォーク型一本道分岐」は016までの横方向帯状
  フィールド＝自由なレーン変更前提の戦闘・建築・タレット配置と根本的に噛み合わないため対象外とした。
...
- サイクル24・4回目（reviews/018-flagship-veinsight-final.md）で最終総括し判定**FIX**。20シード×4戦略
  （maxTicks=20000）・3シード×3倍時間（maxTicks=60000）・P01/P02擬似実プレイ（maxTicks=30000）の
  いずれもv2と完全一致する結果でコアファン仮説「情報投資が経済成長の差になる」の成立を再確認し、
  016までの全システムにも致命的回帰なし（homeDestroyed 0件を通算で維持）。「非対称の情報公開」
  「共鳴チャージ」を本命ゲームでも機能する確定パターンとして採用し、新規投資カテゴリは単体
  プロトタイプ相場より大幅に安い価格帯が必要という教訓を確定させた。残課題はP01の構造的な早期死亡に
  よるA5/A3体感評価の未確認（P01基本点約3.6/5）と、P02のE4をブラウザ実プレイなしに直接評価できない
  こと（P02基本点約3.8/5）の2点（いずれもバグではなく運用上の制約）。次サイクルは004・010以来検証が
  手薄な建築（building）パートへの回帰を提案。ブラウザAIP実プレイは26サイクル連続で未実施
- **サイクル24・3回目（FIX only、レビューなし・詳細はPR本文参照）**: v2引き継ぎ課題のうち#2
  ...
  - `npm run build`・`npm run simulate`（20シード×5戦略、ペルソナ再現4シード）とも正常完了、
    既存戦略への回帰なしを確認
- サイクル24・2回目（reviews/018-flagship-veinsight-v2.md）でv1最優先課題（scanner/charge投資の
  ROIがマイナス）に対応し判定**FIX**。基礎コスト引き下げ（scanner18→11・charge22→12）・共鳴チャージ
  増分強化・検証bot側canDig()の過小評価バグ修正・'planner'専用レーン選択ロジック追加の4点で、
  'planner'が'blind'をavgScore+16.3%・avgMaxDistance+18.9%・avgOreMined+151.6%で上回ることを実証
- サイクル24・1回目（reviews/018-flagship-veinsight-v1.md）で新規実装し判定**FIX**。016の全システムに
  mining側のフォグ（非対称の情報公開）と共鳴チャージを追加したが、017からの価格そのまま移植では
  scanner/charge投資のROIがマイナスとなりコアファン仮説が不成立（'blind'が'planner'を上回る）という
  重大な課題を発見。v2へ持ち越した

## 過去のサイクル23（完了・アーカイブ）

- サイクル: 23
- 要素: **採掘（mining）**。cycle22-final（reviews/016-flagship-emplacement-final.md）の提案(2)を受け、
  014〜016の3世代連続で拠点防衛システム（脅威予告→恒久ステータス投資→配置建築）を掘り下げたことで
  拠点防衛は一区切りついたと判断し、007以降検証が手薄だった「採掘（mining）」パートへ回帰した
  （サイクル23・1回目で決定）
- 対象ゲーム番号: **017-mining-forkshaft**（サイクル23・1回目で決定・実装済み）。003/011の常設ショップ・
  安全マージン公開・地形由来の危険を継承しつつ、新規に「フォーク型の一本道分岐（バンド境界でのみ
  レーン切替可）」「非対称の情報公開（危険度は常時予告・豊富さは探査Lv投資で開放）」「共鳴チャージ
  （探査×ドリル威力×チャージ投資の三位一体シナジー）」「プラチナ鉱石（band3以降）」を追加した
- サイクル23・4回目（reviews/017-mining-forkshaft-final.md）で最終総括し判定**FIX**。20シード×3戦略
  （maxTicks=60000）の再検証とP01（band5狙いのリスクテイクbot）/P02（保守bot）の擬似実プレイ
  （各5シード×maxTicks=60000）を実施し、band5到達性について「バグではなく一方通行の捨て身プレイ
  専用区画として受け入れる」との最終判断を下した（spec.mdに追記）。一方「安全マージンを0まで
  削っても+4%程度しか進めず僅かでも踏み込むと死亡率が崖状に転落する」非連続なリスクカーブは
  P01視点でN6（渋い報酬）に近いと評価し、次の類似設計への教訓として持ち越した。P01基本点約3.9/5・
  P02基本点約3.3/5（P02はE4想像の余地の構造的欠如が主因）。「フォーク型一本道分岐」「非対称の
  情報公開」「三位一体シナジー」を採掘パターンとして採用、次サイクルは本命ゲーム統合フェーズ
  （018-flagship-*）でmining側にこれらのパターンを統合することを提案。ブラウザAIP実プレイは
  22サイクル連続で未実施
- サイクル23・3回目（FIX only、レビューなし・詳細はPR本文とspec.mdの「band5到達性についての
  最終決定」節参照）: v2引き継ぎ#1（band5到達性の検証）に対応し、`headless/simulate.ts`に
  安全マージンを意図的に削る3つ目のbot戦略`pusher`を新規追加した（`src/core/game.ts`は無変更）。
  バッファ0〜2の範囲では死亡0/20を維持したままavgMaxDistanceが160.0(planner)→166.7(pusher)へ
  微増するのが安全に押し切れる限界で、バッファを少しでも負にすると即座に死亡20/20へ崖のように
  転落することを確認。band5(x=201-240)はバッファ0でも166.7止まりで到達不能（燃料タンク最大Lvでも
  往復前提の燃料総量が構造的に足りない設計上の限界）と判明し、4回目での最終判断へ申し送った
- サイクル23・2回目（reviews/017-mining-forkshaft-v2.md）でv1最優先課題（共鳴チャージの発動頻度
  0.6回）に対応し判定**FIX**。上限引き下げ＋鉱石階層比例の増分で20シード平均resonanceTriggersを
  3.7回へ改善。検証過程で発見した検証bot側の2つの致命バグ（詰み記憶の丸ごとクリアによる無限
  ループ／燃料の安全マージン壁での空振り無限出撃）も解消し、20シード×60000tick・5シード×
  150000tickで全シード安定完走（死亡0/20）を確認。全カテゴリ最大強化でも安全にはband4境界
  （x≈160）までしか到達できない設計上の性質も新規発見（バグではなく意図通り、次回検証課題へ）
- サイクル23・1回目（reviews/017-mining-forkshaft-v1.md）で新規実装し判定**FIX**。フォーク型
  一本道分岐・非対称の情報公開・共鳴チャージ・プラチナ鉱石を新規追加し、20シード比較でplanner
  （探査/チャージ投資）がblind（無視）をavgScore+26.6%・avgMoneyEarned+128.9%で上回りコアファン
  仮説を実証、死亡0/20。開発中にヘッドレスbotのフォーク一本道詰みレーンでの無限往復（致命→解消、
  検証bot側のみ）・探査/チャージの初期価格が高すぎて実質購入されない問題（中→解消）を発見・
  修正した。共鳴チャージの発動頻度の低さ（20シード平均0.6回）をv2へ持ち越した

## 過去のサイクル22（完了・アーカイブ）

- サイクル: 22
- 要素: **本命ゲーム統合フェーズ・第六世代（`016-flagship-*`、拠点防衛設備＝トラップ・タレット等の
  新規建築種）**。cycle21-final（reviews/015-flagship-bulwark-final.md）の提案(2)を受け、015の
  仕様書「スコープ外」に明記していた次点候補に着手する。015の`basedefense`は「資金を投じて既存の
  ステータス（自動迎撃ダメージ）を底上げする」抽象的な恒久投資だったが、016では
  **プレイヤーが実際に配置する防衛設備**（トラップ・タレット等の新規建築種）を追加し、
  007〜010で確立した「建築が実際に進行を可能にする」（バリケード＝身代わり、前線拠点＝目標を生む
  建築）というパターンを拠点防衛の文脈にも適用する。`basedefense`（恒久ステータス投資）と対になる
  「建築による防衛投資」を新設することで、「戦闘・採掘・建築」を併せ持つ本命ゲームの"建築"要素を
  拠点防衛にもう一段深く結びつけられるか検証する
- 対象ゲーム番号: **016-flagship-emplacement**（サイクル22・1回目で決定・実装済み）
- サイクル22・4回目（reviews/016-flagship-emplacement-final.md）で最終総括し判定**FIX**。20シード×2戦略
  （maxTicks=20000）・P02長時間ストレス検証（maxTicks=60000、6シード）・P01/P02擬似実プレイ
  （maxTicks=30000）のいずれでもhomeDestroyed 0件を再確認した。014（脅威予告）→015（恒久ステータス
  投資）→016（配置建築）で拠点防衛システムが三本立てで完成したと総括し、次サイクルは拠点防衛系を
  いったん区切って「採掘（mining）」パートへ回帰することを提案。ブラウザAIP実プレイは18サイクル連続
  で未実施
- サイクル22・3回目（FIX only、レビューなし・詳細はPR本文とspec.md「v3で実施した修正内容」節参照）:
  v2の軽微な観察事項#3（ヘッドレスbotのタレット配置ロジックが機械的で「意図的に特定レーンへ寄せる」
  判断をしていない）に対応した。`headless/simulate.ts`のみ変更（game.ts無変更）。防衛志向2戦略
  （cautious・p02）に、拠点の既存タレットレーンから最も離れたレーンを選ぶfarthest-point方式の
  `pickDefensiveTurretLane()`を追加し、狙いのレーンへ移動してから設置するようにした。攻勢志向
  2戦略（pusher・p01）は無変更のまま「意図的配置 vs 機械的配置」の対比を作った。cautiousの
  avgTurretKillsがv2比2.6→6.0・1.6→6.1へ大幅増加、hp死亡も2/10→0/10・1/10へ減少しavgScoreも
  向上。pusher/p01は既存シードでv2と完全一致（回帰なし）。P02×maxTicks=60000（6シード）・
  P01/P02×maxTicks=30000（4シード）ともhomeDestroyed 0件を維持しバランス回帰なし
...
- **サイクル21-finalで確定した運用ルール**: P01ペルソナは013・014・015・016の4ゲーム連続で夜フェーズ
  到達前に前線戦闘で決着し、昼夜サイクル系のコア機構を一度も検証できていない。今後この系統
  （昼夜サイクル・拠点防衛が主軸）のゲームでは「P01は夜フェーズ検証の対象外、コア機構の判定はP02の
  結果のみで行う」ことをデフォルト運用とする（詳細はreviews/015-flagship-bulwark-final.md
  「捨てる・見直すべき部分」参照）

## 過去のサイクル21（完了・アーカイブ）

- サイクル: 21
- 要素: **本命ゲーム統合フェーズ・第五世代（`015-flagship-bulwark`、拠点防衛への恒久投資システム）**。
  cycle20-final（reviews/014-flagship-forewarn-final.md）の提案(2)を受け、014で新規発見したバグ#4
  「想定セッション時間の3倍以上（21夜以降）の極端な長時間プレイで、際限なく上昇するレイダー攻撃力倍率
  （`RAID_NIGHT_ATK_MULT`に上限なし）に対し固定値の拠点自動迎撃（`BASE_AUTO_DEFENSE_DMG=3`）が
  長期的にはスケールせずhomeDestroyedが再発しうる」現象を、単純な定数調整ではなく**プレイヤーが
...
- サイクル21・3回目（FIX only）はv2のバグリスト#2〜4がいずれも対応不要のまま残課題が
  無かったため、v2のLearnings（「新しい投資先を追加したら既存のヒント/予告/UI計算式への反映を
  リストアップして確認する」）に沿ってcomputeBaseForecasts()を再点検し、バグ#5とは独立した
  **新規バグ#6**を発見・修正した: 予告のband係数(mult計算)が拠点自身の座標のband
  （`bandAt(b.x)`、ホームなら常に0）を使っていたが、実際の`spawnRaidWave()`は個々のレイダーの
  湧き元タイルのband（`bandAt(picked.x)`、拠点から離れた深い場所のことが多い）を使っており、
  ホーム拠点の予告が「深く掘るほど強くなるレイダー」の脅威上昇を一貫して過小評価していた
  （014由来、v1/v2ではbasedefense関連のバグ#5にのみ焦点が当たり見落とされていた）。
  拠点ごとの候補タイル群のband加重平均(spawnBand)に置き換えることでspawnRaidWaveの重み付け
  ロジックと数学的に一致させ、DEBUG_FORECAST_BAND一時計測で最大band差1.44を確認、修正後は
  10シード×maxTicks=20000（cautious/pusher）・p01/p02×maxTicks=30000・p02×5シード×maxTicks=60000の
  全回帰セットでv2レビューの値と完全一致（＝予告の表示精度のみ改善し、実際のゲーム挙動には
  無影響）を確認した。詳細はspecs/015-flagship-bulwark/spec.md「3回目で実施した修正内容」参照
  （3回目はレビューを書かない運用のためreviews/には残さない）
- **サイクル21・4回目（FINAL REVIEW、reviews/015-flagship-bulwark-final.md）**: maxTicks=20000の
  cautious/pusherを新規10シード（11〜20）追加して20シードへ倍増し、014-finalの3倍検証
  （maxTicks=60000）も未使用の新規シード（P02の331/332、cautious/pusherの21〜30、計17run）で
  追加検証したところ、既知シード・新規シードともhomeDestroyed 0件を一貫して維持しバグ#4解消の
  信頼度をさらに引き上げた。basedefenseLvはこれらの長時間runでも最大10（maxLevel15には未到達）に
  留まり、v1/v2が「対応不要」と判定したmaxLevel到達後の理論上の限界も今回は観測されなかった。
  P01は013・014・015の3ゲーム連続で夜フェーズに到達できず（既知のペルソナ構造的ミスマッチ）、
  次サイクル以降は「P01は夜フェーズ検証の対象外、コア機構の判定はP02主体」という運用をデフォルト化
  することを提案。判定**FIX**。「拠点防衛への恒久投資」を本命ゲームの確定パターンとして採用し、
  次サイクルは（1）ブラウザAIP実プレイ、（2）015スコープ外の拠点防衛設備（トラップ・タレット等の
  新規建築種）を追加する016-flagship-*を提案した
- ブラウザAIP実プレイは4回（1〜4回目）とも「無人セッションからは開発サーバを起動できない」という
  制約により未実施（`.claude/launch.json`に`bulwark`のdevサーバ設定を追加済み、port 5184）

## 過去のサイクル20（完了・アーカイブ）

- サイクル: 20
- 要素: **本命ゲーム統合フェーズ・第四世代（`014-flagship-forewarn`、拠点防衛の予告システム）**。
  cycle19-final（reviews/013-flagship-nightwatch-final.md）の提案(2)を受け、013の昼夜サイクル・
  拠点HPシステムに、次の夜の脅威規模を昼のうちに知らせる新規危険度ヒント`baseForecasts`
  （拠点ごとの推定被弾リスク）を追加した
- 対象ゲーム番号: **014-flagship-forewarn**（サイクル20・1回目で決定・実装済み）
- サイクル20・4回目（FINAL REVIEW、reviews/014-flagship-forewarn-final.md）で完了。判定**FIX**。
  v1/v2の10シードを20シードへ倍増してもhomeDestroyedは0/20のまま安定していることを確認する一方、
  想定セッション時間の3倍（maxTicks=60000、21夜以降）まで擬似実プレイを延長すると、レイダー攻撃力
  倍率が際限なく上昇する設計に対し固定値の拠点自動迎撃が長期的にはスケールせず、homeDestroyedが
  再発しうる境界事象を新規発見した（バグ#4、想定プレイ時間内では対応不要と判定）。P01は新規4シード
  （合計8シード）を追加してもnightsSurvived平均0.125と013-finalで判定済みの構造的ミスマッチが
  再現し続けることを確認した。ブラウザAIP実プレイは本レビューで初めて実際に`preview_start`を試み、
  無人セッションからは起動できないという明示的な拒否を一次情報として確認した。次サイクルへの提案として
  （1）11サイクル連続で未実施のブラウザAIP実プレイの実施、（2）バグ#4の発見を活かした拠点防衛への
  恒久投資システム（015-flagship-*）を挙げた（詳細は上記「現在位置」参照）
...
- サイクル20・3回目（FIX only、レビューなし・詳細はPR本文とspec.md「v3 FIX内容」節参照）:
  ...
  （`npm run build`・`npm run simulate`とも正常）

## 過去のサイクル19（完了・アーカイブ）

- サイクル: 19
- 要素: **本命ゲーム統合フェーズ・第三世代（`013-flagship-nightwatch`、昼夜サイクル/波状の防衛圧力）**。
  cycle18-final（reviews/012-flagship-frontierline-final.md）の提案(2)を受け、012の横方向帯状
...
- サイクル19・4回目（FINAL REVIEW、reviews/013-flagship-nightwatch-final.md）で完了。判定**FIX**。
  擬似実プレイのmaxTicksを60000へ延長し新規シード（311/312）を追加した結果、コアファン仮説後半
  「今夜どの拠点を見捨てるか」がseed312（前線拠点は守れたがホームが単一の夜(tick10000〜10159)で
  153→-3まで陥落）で初めて実際の敗北を伴って再現された。P02(seed302)は7回の夜すべて
  baseDamageTaken=0という完璧な防衛も達成し、拠点防衛が理不尽な強弱の両極端でないことを実証した。
  一方P01は2セッション（seed301/311）とも夜フェーズ到達前（tick631/804、DAY_LENGTH=1200未満）に
  前線戦闘で決着し、013の新規要素を一度も検証できなかった。これはP01のペルソナ特性
  （E3「軽い緊張を好む」、N16「重い恒久ロストは過剰」）と013のコア機構（ホーム陥落=恒久敗北）の
  構造的ミスマッチと判定し、SCRAP相当ではなく「このペルソナには相性が合わない要素」として記録した。
  ヘッドレスの機械的cautious/pusherは依然homeDestroyed率が高い（20シードでcautious 14/20）が、
  `criticalHp`分岐が隣接1体からも必ず撤退することによる撃破機会の逸失とdashクールダウン枯渇の
  複合が原因という仮説をコードレビューで立てた（未検証、次回持ち越し）。次サイクルへの提案として、
  （1）9サイクル連続で未実施のブラウザAIP実プレイの実施、（2）拠点防衛の予告システム
  （次の夜の脅威規模を昼のうちに知らせる新規危険度ヒント）を追加する014-flagship-*を挙げた
  （詳細は上記「現在位置」参照）
- サイクル19・2回目（FIX＋REVIEW、reviews/013-flagship-nightwatch-v2.md）で判明した事項:
  1. v1致命#1・重大#2・重大#3の3件すべてを修正した: (1)夜明けに残存レイダーを撤退させる、
     ...
     制約から切り離す
  2. **擬似実プレイ（P02, seed302）でv1の主要懸念「複数夜にまたがる累積ダメージによるホーム陥落」が
     実際に解消したことを直接確認した**。v1では同一seedがtick4298で死亡していたが、v2では
     maxTicks(20000)いっぱいまで生存し続けた（夜11回生存）
  3. 一方でヘッドレスの機械的cautious/pusher戦略は改善しつつも（cautiousのhomeDestroyed
...
- サイクル: 18
- 要素: **本命ゲーム統合フェーズ（`012-flagship-frontierline`）**。cycle17-final
  （reviews/011-mining-driftline-final.md）が、009（戦闘）・010（戦闘×建築）・011（採掘）の
  3サイクルを通じて横方向帯状フィールド構造上で戦闘・採掘・建築すべての単体パターンが
  機能することが出揃ったと結論づけ、戦闘・採掘・建築を同時に統合した本命ゲーム試作機
  （008とは別に、横方向帯状フィールド構造を土台にした「第二世代」の位置づけ）への着手を提案した
- 対象ゲーム番号: **012-flagship-frontierline**（サイクル18・1回目で決定・実装済み）
- サイクル18・4回目（FINAL REVIEW、reviews/012-flagship-frontierline-final.md）で完了。
  10シードのヘッドレス比較（cautious死亡0/10、pusher死亡4/10だが単一シード60000tickチェックでは
  生存継続）でv3から回帰がないことを確認した。P01/P02の擬似実プレイでは、P01のmaxDistanceが37で
  頭打ちに見える現象を50tick刻みのx座標トレースで裏取りし、v1のような「壁際で完全に動けない停滞」
  ではなく「撤退HP閾値15%という極端な設定のもとで深追いと撤退を繰り返した末に自己ベストを
  更新できなかった」だけであり、隣接敵数の最大観測値も3（v1/v2の完全スタックのような理不尽な
  集中砲火ではない）と確認した。P02も進行距離225まで到達し停滞なし。v2/v3からの残課題#1
  （集中砲火リスク）はv3の準スタック対策で大幅緩和されたと再確認して解消と判定、#2（pusherの
  barricade効率）は「バリケードで被弾を肩代わりしながら前進する一貫した戦術」でありバグではなく
  戦略選択の結果と最終判断した。判定は**FIX**。本命ゲームに採用すべきパターンとして「掘った道が
  敵の侵攻経路になる空間結合」「バリケードの二重機能（移動＋射線遮断）」「敵スポーンの2段階空間
  分散（重複除外＋距離優先）」「複数の危険度ヒント同居時は発火頻度の横並び比較が必須」を確定し、
  次サイクルへの提案として、7サイクル連続で未実施のブラウザAIP実プレイの実施と、7 Days to Dieの
  核である昼夜サイクル/波状の防衛圧力を012の前線拠点システムに追加する新規プロトタイプ（013-*）を
  挙げた（詳細は上記「現在位置」参照）
- サイクル18・3回目（FIX only、レビューなし・詳細はPR本文とspec.md「v3 FIX内容」節参照）で
  対応した事項:
  1. **【解消】残課題#3（軽微、敵の準スタック）**: `spawnEnemies`の候補マス選定で、既存の敵から
     Chebyshev距離2マス以上離れた候補を優先するよう変更した（`games/012-flagship-frontierline/
     src/core/game.ts`）。離れた候補が無い場合のみ従来通り近接候補にフォールバックする
  2. **効果が残課題#1（中、集中砲火リスク）にも波及**: 準スタックが「隣接する複数の敵に同時に
     攻撃される」状況の主要な発生源だったため、この1箇所の修正だけで20シード比較
     （`--seeds 1..10 --maxTicks 20000`）のcautious死亡率が1/10→**0/10**に改善した。pusherは
     死亡率こそ横ばい（3/10→4/10、通常変動の範囲）だが、avgMaxDistance97.5→108.7、avgScore
     （5シード比較で1345.6→2319.4）と進行・スコア指標が大きく改善した
  3. **試して却下した案**: 残課題#1が提案していた「隣接する敵が一定数以上ならHP閾値未満で
     なくても退く」というペルソナbotロジック（`headless/simulate.ts`）を実装し複数の閾値
     （隣接2体+HP60%未満／隣接2体+HP40%未満／隣接3体+HP50%未満）で検証したが、いずれも
     死亡率の改善は限定的または悪化する一方、avgScore・avgMaxDistance・avgKillsが大幅に
     低下した（例: 隣接2体+HP60%未満ではcautious avgScore2177.0→871.8、pusher1345.6→728.0）。
     退却判断が実際の戦闘機会まで広く放棄させ20000tick内の進行を大きく鈍化させることが
     原因と判明したためrevertし、`headless/simulate.ts`は無変更（HEADと同一）で確定した
  4. **重要な学び**: 「症状（複数の敵に囲まれて死ぬ）」への対症療法（bot側の退却ロジック追加）
     より、「根本原因（敵の出現が同じ場所に偏る）」への対処の方が、副作用なく症状も改善する
     ケースがあると再確認した。bot側のヒューリスティックを追加する前に、まず現象の根本原因が
     ゲーム本体側にないか（今回のように出現ロジックの空間分布等）を疑うべき
  5. 未対応で持ち越し: 残課題#1（bot側ロジックとしては見送ったが、ゲーム本体側での対処余地は
     未検討）・残課題#2（pusherのbarricadesBuilt/Lostが依然高水準、avgBarricadesBuilt123.9）は
     4回目（FINAL REVIEW）で改めて評価すること
  6. リグレッション確認として単一シード`--maxTicks 60000`フル実行チェックを実施し、恒久停止
     （stall）が再発していないこと、seed1 pusherのmaxDistanceがv2と同一の154に到達することを
     確認した
  7. 本セッションも無人のスケジュール実行のため`preview_start`は試みず、ヘッドレスシミュレーション
     による定量検証のみで確認した（コード変更が`spawnEnemies`の候補選定ロジックのみのため、
     擬似実プレイでの再確認は4回目のFINAL REVIEWで両ペルソナ分を実施する）
- サイクル18・2回目（FIX＋REVIEW、reviews/012-flagship-frontierline-v2.md）で対応した事項・
  新たな引き継ぎ事項:
  1. **【解消】致命バグ#1「archerスタンドオフによる進行不能ループ」**: 射線上（プレイヤーと
     遠距離敵の間）にバリケードがあれば、遠距離攻撃はプレイヤーの代わりにバリケードへ命中する
     ようにした（`lineOfSightBarricade`、Bresenhamで射線上の中間マスを走査）。5〜10シードの
     いずれの検証でも「進行距離固定のまま撃破数だけ伸びる」パターンは再発しなかった
  2. **【解消】中バグ#2「combatRiskLevelが常にsafe」**: `recommendedHpForBand`を
     `66+band*12`→`100+band*20`に変更し、band0で初期maxHp(100)とほぼ等しくなるよう
     起点を引き上げた
  3. **【改善】中バグ#3「pusherのbarricade浪費」**: #1の修正でバリケードが射線も防ぐようになり、
     「無駄な浪費」から「機能する防御投資」に性質が変わった（建設数自体は高水準のまま）
  4. **【解消】軽微バグ#4「検証botがdash未使用」**: `headless/simulate.ts`のBotに
     HP危険時の緊急離脱・遠距離敵への間合い詰めの2箇所でdash分岐を追加した
  5. **【新規発見・解消】致命的な新バグA「敵の完全スタック出現による理不尽な集中砲火」**:
     `spawnEnemies`が既に敵がいるマスを候補から除外していなかったため、プレイヤーが足止め
     されている間に同じ座標へ複数の敵が完全に重なって出現し、同時攻撃で理不尽な即死・停滞を
     引き起こしていた（P01の即死・P02の停滞・pusher botの停滞に共通する根本原因と判明）。
     候補マス収集時に既存の敵との重複を除外するよう修正（`games/012-flagship-frontierline/
     src/core/game.ts`の`spawnEnemies`）
  6. **【新規発見・解消】中バグB「移動不能な壁の前でbotが停止し続ける」**: 検証botの
     「敵を追って移動する」ロジックが移動先の前進可否（FLOORか、ドリル威力で掘削可能か）を
     確認していなかった。`canAdvance`ヘルパーを追加し、前進不可なら採掘ロジックへ
     フォールバックするよう修正（`headless/simulate.ts`）
  7. **重要な学び**: 新バグA修正により停滞（over=falseのまま指標凍結）は解消したが、死亡率は
     v1よりやや上昇した（cautious 0/5→1/5、pusher 1/5→3/5、10シードでは3/10）。これは
     「進行不能で足止めされていたケースが、実際に先へ進めるようになった結果、より深いband
     の本物の戦闘難度に晒されるようになった」ことが主因と判定した。**「停滞（詰み）」と
     「戦闘死（正当な敗北）」は質的に異なり、死亡率という数値だけで前後比較すると誤った
     結論に至りうる**——次回以降も死因の質的内訳を必ず確認すること
  8. 残課題（3回目で余力があれば対応検討）: (a)複数の敵に同時に囲まれた際の集中砲火リスクが
     依然として高い（ペルソナbotに「隣接敵数が一定以上なら退く」判断の追加を検討）、
     (b)pusherのbarricadesBuilt/Lostが依然高水準、(c)敵の完全スタックは解消したが
     近接タイルへの「準スタック」は起こりうる
  9. 本セッションも無人のスケジュール実行のため`preview_start`は試みず、core `Game`クラス
     直接step()による擬似実プレイ（P01/P02、v1と同一の交戦距離・撤退HP閾値・ショップ優先度）
     で代替した。一時検証スクリプトはレビュー後に削除済み
- 引き継ぎ事項（cycle18・1回目より、reviews/012-flagship-frontierline-v1.md参照。
  上記2回目で#1・#2・#4は解消、#3は性質改善）:
  1. **【最優先】致命バグ#1「弓兵(archer)スタンドオフによる進行不能ループ」**: バリケードは敵の
     移動のみを塞ぎ射撃は塞がない設計（`stepEnemies`の射程判定`newDist<=e.range`はバリケードの
     有無を見ない）。archerが射程内に留まり続けると接近も撤退もできない停滞ループに陥り、
     P01は死亡（`games/012-flagship-frontierline/src/core/game.ts`のtick500→1400でmaxDistance37
     固定のままHP77→死亡）、P02は40000tick以上完全停止した。修正案はレビュー参照
     （バリケードの遮蔽効果を射撃にも拡張する、archerに回り込みAIを追加する等）
  2. 中バグ#2: `combatRiskLevel`が全シミュレーションで一度も'safe'から動かなかった
     （`miningRiskLevel`は機能）。`recommendedHpForBand`（66+band×12）の再調整が必要
  3. 中バグ#3: pusher戦略の`barricadesBuilt`が219〜384と極端に多く、稼いだ金の大半が
     バリケードの建て直しに溶けて本質的な強化に投資できていない（バグ#1と組み合わさり悪化）
  4. 軽微バグ#4: 検証bot（`headless/simulate.ts`）がdashアクションを一度も使用していない。
     v2でbot側にdash分岐を追加し、バグ#1が人間プレイでも詰むのか・botの実装漏れだけなのかを
     切り分けること
  5. 危険度UIヒント（`combatRiskLevel`/`miningRiskLevel`相当）の評価は探索精度に強く依存すると
     011-finalで再確認された。本命ゲームのAI評価では複数の探索精度のプレイヤー/botを比較する
     手法を標準化すること（本サイクルもP01/P02で優先度・交戦距離・撤退閾値を変えた専用botを使用）
  6. 6サイクル連続でブラウザAIP実プレイが未実施（無人スケジュール実行のため）。人間の対話
     セッションでこのリポジトリを扱う機会があれば、一度はブラウザAIPでの実プレイを試みること

## 過去のサイクル17（完了・アーカイブ）

- サイクル: 17
- 要素: **採掘（mining）**。cycle16-final（reviews/010-combat-building-bastion-final.md）の
  優先案に従い、003-mining-deepvein（縦シャフト、幅16×深さ160）の採掘システムを
  009/010で確立した横方向帯状フィールド構造（幅5レーン×長さ320、band=floor((x-1)/40)）へ
  移植する検証を行った
- 対象ゲーム番号: **011-mining-driftline**（サイクル17・1回目で決定）
- サイクル17・4回目（FINAL REVIEW、reviews/011-mining-driftline-final.md）で完了。
  20シード比較・単一シードのmaxTicksフル実行チェック（20000→60000→150000）ともv2・v3から
  完全に同一の結果を再確認し、v3が検証bot側のみの改修でゲーム本体（`src/core/game.ts`）を
  変更していないことを裏付けた。P01/P02の擬似実プレイは、v3で確立した危険タイル回避ロジックを
  持つ`headless/simulate.ts`のBot.decide()を流用しショップ優先度だけをペルソナ別に差し替える
  統一手法で実施し、両者とも危険タイル被弾ゼロで生存し続け、致命バグ#1（経済成長の恒久停止）の
  再発なくtick数を伸ばすほど指標が前進し続けることを確認した。判定は**FIX**。003で確立した
  常設ショップ・安全マージン公開・地形由来の危険・ビルド選択の結果責任の4パターンが横方向帯状
  フィールドへの移植後も機能することを実証し、009（戦闘）・010（戦闘×建築）・011（採掘）の
  3サイクルを通じて要素ローテーションの全ピースが横方向帯状フィールド構造で出揃ったと結論づけ、
  次サイクルは戦闘・採掘・建築を同時に統合する本命ゲーム試作機（012-flagship-*）への着手を
  提案した（詳細は上記「現在位置」参照）
- サイクル17・3回目（FIX only、レビューなし・詳細はPR本文とspec.md「v3 FIX内容」節参照）で
  対応した事項:
  1. v2バグリストの残課題2件（中バグ#2「`miningRiskLevel`再悪化ハイライトの過剰発火」、
     軽微バグ#3「5レーンの迂回余地」）を調査した。両方とも`headless/simulate.ts`のcautious/pusher
     botがタイル種別を一切考慮せず最初に見つけた掘削可能タイルへ進んでいた（危険タイルも安全タイルも
     区別しない）ことが疑わしい共通原因だったため、同一優先順位内で危険タイル(GAS/UNSTABLE)を
     避けられるレーンがあれば優先する迂回ロジックを追加した
  2. **【解消・実証】軽微バグ#3（5レーンの迂回余地）**: 20シード比較で危険タイル被弾数が
     avgHazardHits 0.55〜0.60回→**0.00回**まで減少し、5レーンでも迂回する意思決定さえあれば
     ほぼ完全に回避できることを直接確認した。`LANES`の変更は不要と判断
  3. **【対応不要と判断】中バグ#2（`miningRiskLevel`再悪化ハイライトの過剰発火）**: 迂回ロジック
     追加後もavgRiskEscalations/avgTripsの比はほぼ変化しなかった（cautious2.01→2.00、
     pusher2.04→2.04、単一シードの`--maxTicks 20000/60000/150000`フル実行チェックでも一貫して
     約2.0回/トリップ）。往路で燃料が減りsafe→caution→dangerを1回ずつ跨ぎ帰還・給油でsafeへ戻る
     という設計上、1往復あたり最低2回のエスカレーションが発生するのが自然な下限であり、bot側の
     探索精度を上げても頻度は変わらないことを実測で確認した。009/010の`combatRiskLevel`も同一
     実装（severityが上がるたびに毎回発火）で問題視されていないことと整合するため、**過剰発火では
     なく設計通りの頻度**と判断し、ゲーム本体（`src/core/game.ts`）は無変更とした
  4. リグレッション確認として単一シードの`--maxTicks 20000/60000/150000`フル実行チェックを
     再実行し、v2で解消した致命バグ#1（経済成長の恒久停止）が迂回ロジック追加後も再発していない
     （tick数を伸ばすほどmaxDistance・oreMined・upgradesBought・moneyEarnedが確実に増加する）
     ことを確認した
  5. 実行履歴テーブルにサイクル17・run1/run2の行が漏れていたことに気づき、本回のPRで遡って追記した
  6. 本セッションも無人のスケジュール実行のため`preview_start`は試みず、これまでと同じく
     `window.__AIP__`と同一のcore `Game`クラス直接step()による擬似実プレイ相当の検証
     （ヘッドレスシミュレーションでの定量検証）で代替した。ヘッドレス以外の直接検証（P01/P02の
     擬似実プレイ）は今回変更したのがテストハーネス（`headless/simulate.ts`）のみで
     ゲーム本体は無変更のため実施していない。4回目（FINAL REVIEW）で改めて両ペルソナの
     擬似実プレイを行うこと
- サイクル17・2回目（FIX＋REVIEW、reviews/011-mining-driftline-v2.md）で対応した事項・
  新たな引き継ぎ事項:
  1. **【解消】致命バグ#1「経済成長の恒久停止（停滞トラップ）」**: 2つの修正を併用して解消した。
     (a) タイルごとの掘削進捗の永続化（`digProgress`）——危険域境界で撤退しても次回同じタイルの
     残りtickから再開できるようにした。(b) 「詰みからの脱出手段」（008パターン#4／010 v2の
     移植、`stuckIncome`）——ホーム滞在中、最安の未購入強化すら買えない間だけ25tickごとに2金の
     セーフティネット収入が入るようにした。**(a)単体では境界がちょうど1マスだけ前進して
     再び恒久停止することが判明**（境界到達時に掘削1tickの猶予が残っているかは偶然に左右され、
     猶予ゼロの新しい均衡点にずれるだけだったため）。(b)を併用し「装備強化の原資を必ず得られる」
     ようにして初めて、tick数を伸ばすほどmaxDistance・oreMined・upgradesBought・moneyEarnedが
     確実に増加する（`--maxTicks 20000/60000/150000`のフル実行チェックで確認）状態に転換できた。
     20シード比較でも全指標がv1から改善（cautious avgScore183.5→209.2、avgMaxDistance51.1→58.3等）
  2. **新規知見**: フルアップグレード後（全8カテゴリ・全レベル購入済み）に固定往復軌道へ収束し
     指標が完全停止する現象を確認した（P01 seed301、tick20000〜150000で全指標不変）。これは
     致命バグ#1（本来買えるはずの強化すら買えず低レベルのまま詰む）とは性質が異なる
     「フルスペックでの自然な上限到達」であり、対応不要と判断してLearningsに記録するに留めた。
     `upgradesBought`が全カテゴリの`maxLevel`合計に達しているかで両者を切り分けられる
  3. 中バグ#2（`miningRiskLevel`再悪化ハイライトの過剰発火）・軽微バグ#3（5レーンの迂回余地）は
     今回未対応のまま持ち越し（優先度に従い致命のみ必須対応とした）。3回目（FIX only）で
     余力があれば対応を検討すること
  4. 本セッションも無人のスケジュール実行のため`preview_start`は試みず、003・009・010・
     011v1と同じく`window.__AIP__`と同一のcore `Game`クラス直接step()による擬似実プレイで
     代替した
- サイクル17・1回目（BUILD＋REVIEW、reviews/011-mining-driftline-v1.md）で判明した事項:
  1. 003が確立した4パターン（常設・複数カテゴリショップ、安全マージンの数値公開
     `estFuelToReturn`、地形由来の危険だけで成立する難易度、ビルド選択の結果責任）は、
     横方向帯状フィールドへ移植しても定性的に同じ強さで機能することをP01（seed301、死亡）・
     P02（seed302、健在）の擬似実プレイ対比で確認した。008の11パターンのうち戦闘由来9つ
     （009final）・建築由来2つ（010final）に続き、採掘由来パターンも空間構造非依存の
     汎用原則であることが確認され、要素ローテーションの最後のピースが埋まった
  2. 致命バグ#1「経済成長の恒久停止（停滞トラップ）」を発見（詳細・解消は上記2回目の記録を参照）
...
- サイクル16・4回目（FINAL REVIEW、reviews/010-combat-building-bastion-final.md）で完了。
  20シード×8戦略のヘッドレス比較・単一シード60000tickフル実行チェック・両ペルソナ擬似実プレイの
  いずれもv2から完全に同一の結果（tick・スコア・イベント系列まで一致）を再現し、v3のrevertが
  コードを正しくv2時点に戻したことを実測で確認した。判定は**FIX**。前線拠点（目標を生む建築）を
  「008・009・010の3ゲームを通じて最も汎用性・効果の大きい設計パターン」として本命ゲームの中核に
  採用すべきと結論づけ、次サイクルは採掘要素の横展開検証（011-mining-*）を優先案として提案した
  （詳細は上記「現在位置」参照）
...
- サイクル16・2回目（FIX+REVIEW、reviews/010-combat-building-bastion-v2.md）で対応した事項・
  ...
  1. 【解消】v1バグ#3（重大、no-buildが20シード中0/20勝利・50000tickでも停滞する「絶対に
     勝てない壁」）: 拠点圏外・非戦闘中限定のHP版「詰みからの脱出手段」（008パターン#4の拡張、
     15tickごとに+1、拠点回復2/tickの1/30の速度）を追加。maxTicks=60000まで許容すると
     no-buildが10/10勝利に転換し（平均48938tick、outpost-rushの約3.8倍）、「壁」から
     「非効率だが到達可能」に変わった。コア仮説（建築が進行を可能にする）はむしろ定量的に強化された
  2. 【改善】v1バグ#4（中、バリケード単体がno-buildより悪化する逆効果）: `BARRICADE_BASE_COST`
     8→5、`BARRICADE_HP`18→26に調整。avgDistance差が-13.5→-4.5に縮小し、破壊率も74%→約33%に
     低下、no-buildとほぼ拮抗する水準まで改善した
  3. 【解消】v1バグ#5（軽微、balanced+builder方針のP02が想定15〜20分に対し実測約37分と超過）:
     `BASE_REGEN_PER_TICK`1→2に倍増。P02の擬似実プレイが約16.2分に短縮され想定範囲内に収まった
  4. 【新規発見・v3以降で判断】単一カテゴリ隔離の診断ボットdefense-allin（offenseに一切投資
     しない）が10シード中10シードでmaxTicks=60000でも`over=false`のまま停滞（勝利も死亡もしない）
     ことが判明。攻撃力が伸びずband上昇に殺傷速度が追いつかなくなることが原因。ただし実運用の
     balanced系戦略では発生しない診断ボット特有の性質のため、今回は仕様変更を見送りLearningsに
     記録するに留めた
  5. v1のP01ペルソナ戦略「offense特化（防御ゼロ）+outpost-rush」は、v2の複数シード再検証で
     11シード中11シード死亡する事実上の即死ビルドだったと判明（v1のseed301勝利は偶然だった）。
     v2ではatk偏重2:防御1のハイブリッドビルドに変更して再測定し、両ペルソナとも想定時間内
     （P01約15.4分・P02約16.2分）で完走を確認した
  6. 本セッションもスケジュール実行（無人）のため`preview_start`でのdevサーバ起動は試みず、
     009・010v1と同じくheadlessの`Game`クラス直接step()による擬似実プレイで代替した
...
| 2026-07-11 | 1 | 2（FIX+REVIEW） | v1指摘（致命・重大）を修正: 同時湧き上限を全ウェーブ-1体、プレイヤーHP/攻撃力/攻撃速度を強化、旋風斬りCD短縮＋スキル使用回数の計測を追加。simulate/ブラウザAIPで再評価しv2レビュー作成。判定FIX（生存ウェーブ・スキル発動とも改善したが、棒立ちがactiveより成績が良い新規の逆転現象を発見、v3で対応） | #104 |
| 2026-07-11 | 1 | 3（FIX only） | v2指摘（重大・新規）の棒立ち>active逆転現象を修正: 敵の移動先をプレイヤー隣接4マスの「囲みスロット」へ分散割り当てし、同一経路への渋滞（実質1体ずつしか同時接敵できない状態）を解消。あわせてプレイヤーに「移動直後2tickの被ダメージ-15%」を付与し能動的な位置取りへ誘因を付与。この2つの修正で1シードが被ダメージほぼ無しの無限スノーボール（wave19到達もゲーム終了せず）を起こしたため、ウェーブ6以降の敵HP/攻撃力に線形倍率（wave-5を超えた分に0.35/wave）を追加し対処。結果、20シード比較でavgWaves passive3.4/active3.2・avgScore passive809/active768と、v2のpassive3.0>active2.3（大差）からほぼ互角に改善。npm run build / npm run simulate とも正常終了、全シード death=100%維持（無限化なし） | #104 |
...
書かず（3回目FIX onlyの規約通り）、両指摘とも解消済みとしてfinal（次回）へ引き継ぐ | #104 |
...
| 2026-09-04 | 22 | 2（FIX+REVIEW） | v1レビューの軽微な観察事項2件に対応。(1)タレット設置数がほぼ毎ランMAX_TURRETS_PER_BASE付近まで到達し「予算が足りず諦める」場面が観測できない問題へ、他のショップ項目（`SHOP_DEFS`）と同じ複利成長パターンを導入した`TURRET_COST_GROWTH`(=1.6)で対応。同一拠点内の設置数に応じて1基目40→2基目64→3基目102と段階的に高くなるようにした。(2)`TURRET_RANGE=2`がLANE_COUNT=5の中央に置けば全レーンをカバーでき空間トレードオフが検証できない問題へ、射程を2から1へ縮小して対応。中央（y=2）に置いても隣接2レーン（y=1〜3）までしか守れなくなり、両端レーン（y=0,4）は追加配置が必須になった。修正後もcautious/pusher20シード×2・maxTicks=20000、P02×maxTicks=60000（6シード）、P01/P02×maxTicks=30000（4シード）のいずれもhomeDestroyed 0件を維持し回帰なし。avgTurretsBuilt（cautious 1-10: 5.6→4.4）・avgTurretKills（同: 11.6→2.6）はいずれも減少し「作れるだけ作る」自動購入から脱却、一部シード（P02 seed301/311）でbaseDamageTaken増加という形で空間トレードオフの効果が定量的に可視化され始めたことを確認した。新規の軽微な観察事項1件（ヘッドレスbotの配置戦略が依然「意図的にレーンへ偏らせる」判断をしていないため、botが実際に空間ミスを犯すシナリオは未検証）をv3以降へ持ち越した。ブラウザAIP実プレイは`preview_start`を試み17サイクル連続で同一の拒否を再確認。npm run build / npm run simulateとも正常終了。reviews/016-flagship-emplacement-v2.md作成、判定FIX。spec.mdに「v2で実施した修正内容」節を追記、package.jsonのversionを0.2.0へ、games/README.mdの索引を更新し、routine-state.mdをサイクル22・run3（FIX only）へ進めた | #104 |
| 2026-09-04 | 22 | 3（FIX only） | v2観察事項#3（ヘッドレスbotのタレット配置ロジック`pickTurretDir`が「その場で空いている方向に機械的に設置」するだけで、意図的に特定レーンへ寄せる判断をしていないため、v2の射程縮小で理論上作れたはずの空間ミスのシナリオを実際には検証できていない問題）を修正。ゲーム本体（`src/core/game.ts`）は無変更、`headless/simulate.ts`のみ変更。防衛志向の2戦略（cautious・p02）に、その拠点の既存タレットのレーン(y)群から最も離れたレーンをfarthest-point方式で選ぶ`pickDefensiveTurretLane()`を新設（既存レーンが無ければ中央y=2、以降{0,2,4}の順で3基＋TURRET_RANGE=1・LANE_COUNT=5なら全5レーンをカバーできる）し、狙いのレーンと現在地がずれていれば先に上下移動してから設置するようにした。攻勢志向の2戦略（pusher・p01）は従来通り無変更とし、「意図的な配置」対「機械的な配置」の対比を作った。検証: pusher/p01は既存シードでv2と完全一致（回帰なし）を確認。cautiousは10シード×2（1-10, 11-20）・maxTicks=20000でavgTurretKillsがv2の2.6/1.6から6.0/6.1へ大幅増加し意図的配置の効果を定量確認、hp死亡も2/10→0/10・1/10へ減少しavgScoreも向上。P02×maxTicks=60000（6シード）・P01/P02×maxTicks=30000（4シード）のいずれもhomeDestroyed 0件を維持し015・v1・v2が確立した拠点防衛バランスへの回帰なし（p02のturretKillsも軒並み増加、例seed301は6→9、seed312は8→13）。npm run build / npm run simulateとも正常終了。3回目FIX onlyの規約通りレビューは書かず、spec.mdに「v3で実施した修正内容」節を追記した。判定は次回（4回目FINAL REVIEW）で総括、routine-state.mdをサイクル22・run4（FINAL REVIEW）へ進めた | #104 |
| 2026-09-04 | 22 | 4（FINAL REVIEW） | 016-flagship-emplacementの総括レビュー（reviews/016-flagship-emplacement-final.md）を作成。同一20シード・P02長時間ストレス検証（maxTicks=60000、6シード）・P01/P02擬似実プレイ（maxTicks=30000）のいずれでもhomeDestroyed 0件を再確認し、014（脅威予告）→015（恒久ステータス投資）→016（配置建築）で拠点防衛システムが三本立てで完成したと総括、判定FIX。P01は013〜016の4ゲーム連続で夜フェーズ到達前に前線戦闘で決着（既知の構造的ミスマッチ、P02主体判定を継続）。ブラウザAIP実プレイは18サイクル連続で無人セッション制約により未実施。拠点防衛系は014〜016で一区切りとし、次サイクルは007以降検証が手薄だった「採掘（mining）」パートへの回帰を提案、routine-state.mdをサイクル23・run1へ進めた | [#89](https://github.com/mannshu7-droid/gametest-scrapbuild/pull/89) |
| 2026-09-05 | 23 | 1（BUILD+REVIEW） | cycle22-finalの提案を受け、007以降検証が手薄だった採掘（mining）パートへ回帰し`017-mining-forkshaft`を新規実装。003/011の常設ショップ・安全マージン公開・地形由来の危険を継承しつつ、新規に「フォーク型の一本道分岐」（6バンド×3レーン、レーン切替はバンド境界のフォークでのみ可能）・「非対称の情報公開」（危険度は探査Lv0でも常時予告、豊富さは探査Lvへの投資で開放。003/011の「地図は常時全公開」から意図的に変更）・「共鳴チャージ」（鉱石採掘でチャージが貯まり満タン時に次の1手で同xの他レーンも巻き込み採掘、探査×ドリル威力×チャージの三位一体シナジーを検証）・「プラチナ鉱石」（band3以降）を追加した。開発中に2件の問題を発見・解消: (1)ヘッドレスbot（`headless/simulate.ts`）が「フォーク一本道の詰みレーン」に対応できず1マス手前と壁の間を無限往復する偽の停滞を起こしていたのを、詰みレーンの記憶と全滅時のホーム待機ロジックで解消（ゲーム本体`src/core/game.ts`は無変更、003/011のLearnings「単純なbotロジックはゲーム本体のバグと見分けがつかない偽の停滞を生む」の再実証）。(2)探査・共鳴チャージの初期価格（35/40）が他の安価カテゴリに資金を先取りされ続け150000tickの長時間検証でも一度も購入されなかったため、探査18・チャージ22へ改定してspec.mdを更新した。20シード×2戦略（blind=探査/チャージ無視、planner=投資してヒント活用）のヘッドレス比較で、plannerがblindをavgScore+26.6%・avgMoneyEarned+128.9%・avgMaxDistance+30.6%で上回りコアファン仮説を実証、死亡0/20。P01/P02擬似実プレイ（一時スクリプト、レビュー後削除）でも危険タイル被弾・理不尽死は0件のまま、豊富さ偏重が高難度鉱石の混在リスクを伴う設計トレードオフを確認した。共鳴チャージの発動頻度が低い（avgResonanceTriggers 0.6）ことを次回優先の持ち越し課題とした。npm run build / npm run simulateとも正常終了。ブラウザAIP実プレイは`preview_start`を試み19サイクル連続で同一の拒否を再確認（`.claude/launch.json`にport 5186で`forkshaft`設定を追加済み）。reviews/017-mining-forkshaft-v1.md作成、判定FIX。games/README.mdの索引を更新し、routine-state.mdをサイクル23・run2（FIX+REVIEW）へ進めた | #104 |
| 2026-09-05 | 23 | 2（FIX+REVIEW） | v1最優先課題（共鳴チャージの発動頻度が低い、20シード平均resonanceTriggers 0.6回）に対応し、上限チャージをLv1=6→4・Lv2=8→6・Lv3=10→8へ引き下げ、チャージ増分を鉱石種別によらず一律+1から鉱石階層に比例（銅+1・鉄+2・金+3・プラチナ+4）へ変更した。この変更を20シードで検証する過程で、v1が単一シード（seed=1）でしか行っていなかった長時間チェック（maxTicks=150000）を5シードに増やして実行したところ、**v1では発覚していなかった検証botの2つの致命的な意思決定バグ**を新規発見した: (A)「ドリル威力が変わるたび全バンドの詰みレーン記憶を丸ごとクリアする」設計のため、3レーン中2レーンを詰みと確認した直後に強化を1回買うと記憶が消え、正しい3レーン目に永遠に到達できない無限ループに陥っていた（seed=2,4のplannerがmaxDistance=62/63で永続停滞）。レーンごとに「詰まった時点の要求ドリル威力」を記憶し現在の威力で解消したかだけを判定する方式に修正。(B)ドリル威力は足りているのに燃料の安全往復距離の壁で新しい鉱石に届かなくなると「詰み」と認識されず、既に掘削済みの床を往復するだけの空振り出撃を無限に繰り返し、詰みからの脱出手段（stuckIncome）が発動しないまま経済が実質停止していた。「直近の外出で前進距離・採掘量のどちらも伸びなければホーム待機に切り替え、強化購入で即再挑戦する」ロジックを追加して解消。いずれも`headless/simulate.ts`のみの修正でゲーム本体（`src/core/game.ts`）は無変更。修正後は20シード×20000tickでplannerのavgResonanceTriggersが0.6→3.7（約6倍）、avgScore211.6→403.1（+90.5%）、avgMaxDistance56.8→106.8（+88%）と大幅改善し、20シード×60000tick・5シード×150000tickでも全シードが死亡0のまま経済完走（upgradesBought上限42到達）することを確認した。副次的に「全カテゴリ最大強化しても安全にはband4境界（x≈160）までしか到達できず、band5(x=201-240)へはリスク回避戦略では到達不能」という設計上の性質も判明（バグではなく`estFuelToReturn`の安全マージン判定が意図通り機能した結果、次回以降の検証課題として記録）。v1課題#2（豊富さの高いレーンほど高難度鉱石が混じりやすい）はボット修正後20/20シードで詰みからの回復が正しく機能したため対応不要と判断した。P01/P02擬似実プレイ（一時スクリプト、レビュー後削除、5シードずつ）でもv1の「外れの世界」seed303/305含め共鳴発動1〜7回・死亡0件を確認。npm run build / npm run simulateとも正常終了。ブラウザAIP実プレイは`preview_start`を試み20サイクル連続で同一の拒否を再確認。reviews/017-mining-forkshaft-v2.md作成、判定FIX。spec.mdの共鳴チャージ表を更新、games/README.mdの索引を更新し、routine-state.mdをサイクル23・run3（FIX only）へ進めた | #104 |
| 2026-09-06 | 23 | 3（FIX only） | v2の残課題（#1、band5到達性の検証、参考・次回以降検討扱い）に対応。`src/core/game.ts`は無変更、`headless/simulate.ts`のみ変更。plannerと同じ購入優先度・レーン選択を使いつつ、帰還判断だけ`miningRiskLevel==='danger'`（安全マージン15込み）を無視し`estFuelToReturn`ちょうど＋バッファ0まで安全マージンを意図的に削って前進する3つ目のbot戦略`pusher`を新規追加した。20シード×60000tickで検証したところ、バッファ0〜2の範囲では死亡0/20を維持したままavgMaxDistanceがplannerの160.0から166.7へ微増するのがリスクを取って安全に押し切れる限界で、バッファを少しでも負（-5等）にすると死亡率が即座に20/20へ崖のように転落する（中間の緩やかなリスク/リターン曲線が存在しない）ことを確認した。さらに**band5(x=201-240)はバッファ0でも166.7止まりで到達不能**であることが判明——これは安全マージンの調整では解決しない、燃料タンク最大Lv(6)の最大燃料340に対し往路（掘削コスト込みで割高）＋復路（床のみで距離×1）の物理的な燃料総量が構造的に足りないという設計上の限界であり、往復前提である限りどれだけリスクを取ってもband5には届かないことをheadless検証で実証した。この発見をroutine-state.mdの引き継ぎ事項に記録し、4回目（FINAL REVIEW）でband5を「一方通行の捨て身プレイ専用区画」として意図通りと判定するか、燃料タンクmaxLevel引き上げ等の設計変更をすべきか両ペルソナ視点で最終判断するよう申し送った。npm run build / npm run simulateとも正常終了。3回目FIX onlyの規約通りレビューは書かず、routine-state.mdをサイクル23・run4（FINAL REVIEW）へ進めた | #104 |
