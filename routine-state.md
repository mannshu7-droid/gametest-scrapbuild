...
- 003-mining-deepvein v1（reviews/003-mining-deepvein-v1.md）からv2（次回FIX）への引き継ぎ事項:
  1) 【最優先・致命】ドリル威力ゲート内側のtier0タイル（土・銅鉱石）を掘り尽くすと次のドリル強化を買う
     収入源自体が尽きて詰みに近い停滞に陥る。tier1タイル（岩・ガス）のband0要求ドリル威力を1に緩和する、
     または出発地点近傍に一定数のtier0タイルを保証する生成ロジックを入れて対処する、
  2) 【最優先・致命】「残燃料25%で帰還」の閾値が実際の帰還距離を考慮しないため、燃料切れ時のHP-3/tick
     ペナルティで帰還中にほぼ確実に死亡する（fuelEmptyTicksがほぼ一律34=102ダメージ）。bfsで概算した
     帰還所要tickを閾値に織り込むか、燃料切れペナルティ自体を緩和する、
  3) 上記2つを修正した上でshallow/diver比較を再実行し、「動くと得をするか」を正しく測定し直す必要がある
     （今回はバグの影響で両戦略に差が出にくくなっていた）、
  4) ショップ購入回数（現状平均2.0回）を仕様目標の5〜6回まで引き上げられているか、修正後に再確認する
