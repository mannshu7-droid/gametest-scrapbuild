...
- サイクル17・1回目（BUILD＋REVIEW、reviews/011-mining-driftline-v1.md）で判明した引き継ぎ事項:
  ...
  2. **【最重要・v2で必須対応】致命バグ#1「経済成長の恒久停止（停滞トラップ）」**:
     `miningRiskLevel`の危険域境界（初期ステータスのままだとx≈40前後）に到達すると、
     掘削放棄で進捗（`digging`の`remaining`）が破棄され、次回同じタイルへの掘削がゼロから
     やり直しになる無限ループが発生する。単純botは`x≈41`、ore優先の賢いbot(P01/P02相当)でも
     `x≈80〜99`で必ず発生し、一度発生するとtick数を7.5倍（20000→150000）に伸ばしても
     money・oreMined・upgradesBought・maxDistanceが一切動かない。**003本体でも同一の
     `--maxTicks 60000`チェックで再現する003由来の潜在バグ**（009/010final確立の
     「単一シードのmaxTicksフル実行チェック」が003より後にできたため今回まで未発見だった）。
     v2ではこの停滞トラップの解消を最優先（010 v2の「拠点圏外のHP版・詰みからの脱出手段」や
     003 v1→v2の「ハード進行ゲートの緩和」に相当する構造的な修正を検討すること）
  3. 中バグ#2: `miningRiskLevel`のHUDハイライト（`riskEscalationBanner`）は単純な移動bot
     （cautious/pusher）だと1トリップあたり1.7〜2回相当と頻度過多、ore優先の賢いbotだと
     約0.2回と妥当になる。UIヒントの評価は複数の探索精度のbotで比較する必要がある
     （003final Learnings「指標が悪ければまずbot側の探索精度を疑う」の再確認）
  4. 軽微バグ#3（未検証）: 5レーンが003の16マス幅に比べ「危険タイルを迂回する余地」を
     減らしている可能性があるが、本レビューのbotでは危険タイル被弾がほぼ0件で切り分け
     られていない。次回以降、迂回優先度を上げたbotとの比較を検討
  5. 本セッションも無人のスケジュール実行のため`preview_start`は試みず、003・009・010と
     同じく`window.__AIP__`と同一のcore `Game`クラス直接step()による擬似実プレイで代替した
