...
- 003-mining-deepvein v2（reviews/003-mining-deepvein-v2.md）からv3（次回FIX only）への引き継ぎ事項:
  1) 【中】ショップ購入回数が仕様目標（最低5〜6回）に対しshallow平均2.7回・diver平均3.4回とまだ不足。
     死亡・停滞は解消済みなので詰み要因ではなく、積載満杯（帰還トリガー）に到達するまでの収入効率不足が
     原因と推測される。初期積載量・鉱石出現率・鉱石価値のいずれかを見直して1トリップあたりの稼ぎを
     底上げする方向で調整する（ドリル威力ゲートの再調整は詰み再発リスクがあるため避ける）、
