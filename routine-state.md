...
- サイクル: 23
- 要素: **未定（サイクル23・1回目で決定）**。cycle22-final（reviews/016-flagship-emplacement-final.md）の
  提案(2)を受け、014〜016の3世代連続で拠点防衛システム（脅威予告→恒久ステータス投資→配置建築）を
  掘り下げたことで拠点防衛は一区切りついたと判断し、次サイクルは007以降検証が手薄だった
  **「採掘（mining）」パートの深さ**（例: 深度に応じた鉱石の多様化、ドリル強化とビルドのシナジー、
  採掘リスクと報酬のトレードオフ）へ回帰することを提案する。1回目で具体的な仕様書名・ゲーム番号
  （`017-mining-*`または`017-flagship-*`のいずれの位置づけにするか含む）を決定すること
- 対象ゲーム番号: 未定（サイクル23・1回目で`017-*`として決定）
- 次に行う回: **1回目（BUILD＋REVIEW）**
- **【最優先・18サイクル連続の技術的負債、引き続きプラットフォーム側の制約として確認済み】
  ブラウザAIP実プレイが012final〜016-finalを含め直近18サイクル連続で未実施。サイクル22・4回目
  （FINAL REVIEW）でも`preview_start`を試みたが「無人セッション（スケジュールタスク実行）からは
  開発サーバを起動できない（承認者不在のため）」という明文化された拒否を再度受け取った
  （18回目の再確認）。無人スケジュール実行が続く限り構造的に解消しないため、人間の対話セッションで
  このリポジトリを扱う機会があれば最優先で実施すること**
- **サイクル21-finalで確定した運用ルール（引き続き適用中、ただし次サイクルが採掘パートへ回帰する
  場合は見直しを検討）**: P01ペルソナは013・014・015・016の4ゲーム連続で夜フェーズ到達前に前線戦闘で
  決着し、昼夜サイクル系のコア機構を一度も検証できていない。今後この系統（昼夜サイクル・拠点防衛が
  主軸）のゲームでは「P01は夜フェーズ検証の対象外、コア機構の判定はP02の結果のみで行う」ことを
  デフォルト運用とする（詳細はreviews/015-flagship-bulwark-final.md「捨てる・見直すべき部分」参照）。
  一方、採掘パート中心のゲームではP01の評価軸（A1経済・A5仮説検証等）が本来強く効くはずのため、
  次サイクルではP01を主たる判断材料に戻すことも検討する（reviews/016-flagship-emplacement-final.md
  「次にどのようなゲームを作ればよいか」参照）
