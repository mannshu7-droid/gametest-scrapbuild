...
export class Renderer {
  ...
  draw(s: GameState): void {
    ...
    ctx.fillText(
      `kills ${s.metrics.kills}(内レイダー${s.metrics.raidersKilled}/タレット撃破${s.metrics.turretKills})  oreMined ${s.metrics.oreMined}  outposts ${s.metrics.outpostsBuilt}(喪失${s.metrics.outpostsLost})  barricades ${s.metrics.barricadesBuilt}  turrets ${s.metrics.turretsBuilt}(喪失${s.metrics.turretsLost})  score ${s.metrics.score}`,
      6,
      hudY + 160,
    );
    ctx.font = '10px monospace';
    ctx.fillText(
      '1攻撃 2機動 3耐久 4ドリル 5燃料 6採掘速度 7ランタン 8危険耐性 9積載 0テレポート解禁 Q拠点防衛 Pタレット設置',
      6,
      hudY + 178,
    );
