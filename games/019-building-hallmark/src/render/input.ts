 ...
 * F=向いている方向へ設置（lotIndex省略=キュー先頭の資材を消費）、G=向いている方向を撤去、
 * B=地上で選択中資材を購入、P=地上で鑑定レベルを1段階購入、R=リスタート
 */
export class Input {
  ...
  constructor() {
    window.addEventListener('keydown', (e) => {
      ...
      if (['f', 'g', 'b', 'p', '1', '2', '3', '4', '5'].includes(k)) {
        this.queued.push(k);
        e.preventDefault();
      }
      this.pressed.add(k);
  ...
  poll(): Action {
    ...
    if (single) {
      ...
      if (single === 'b') return { type: 'buy', item: this.material };
      if (single === 'p') return { type: 'buy', item: 'appraiser' };
    }
