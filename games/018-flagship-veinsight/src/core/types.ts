...
export type Action =
  ...
  | { type: 'build'; target: 'turret'; dir: Dir }
  | { type: 'teleport' }
  | { type: 'wait' };
...
export const ACTION_SPEC: ActionSpecEntry[] = [
  ...
  },
  {
    type: 'buy',
    params: {
      item: 'offense|mobility|vitality|drill|fuel|digspeed|lantern|hazardresist|capacity|teleport|basedefense|scanner|charge',
    },
    description:
  ...
  {
    type: 'build',
    params: { target: 'turret', dir: 'up|down|left|right' },
    description:
      '隣接する既に掘った道(dir)に拠点防衛タレットを設置（016新規）。拠点（ホームor前線拠点）保護半径内のみ・' +
