 ...
 * - 022新規: 020-finalの発見（防衛系の死亡94%は夜のフィールド、拠点内は7.5%）に対応するため、
 *   タレットのパトロール圏（帰路支援）・鑑定投資による帰還危険度precisionを追加した。検証のため2戦略を新設:
 *   patroller=mason（appraisal＋クラスタ配置）と同じ投資だが、夜の帰還時に`recommendedReturnLane`
 *   （パトロール圏がカバーするレーン）へ意図的に寄る。mason（同じ投資だがレーンには寄らない）との比較で
 *   「パトロール圏へ意図的に寄る」ことの効果だけを分離して検証する。
 *   bare=pusherと同じ攻勢だが、タレットを一切設置しない（パトロール圏投資ゼロの対照群）。他の全戦略
 *   （タレットを多少は建てる）との比較で「タレット投資の有無」が`deathPhase`内訳（特にnight-field比率）に
 *   効くかを検証する
 */
import { Game, FIELD_WIDTH, LANE_COUNT, LENGTH, BAND_SIZE, bandAt, requiredDrillPower } from '../src/core/game';
import { TILE, type Action, type DeathPhase, type Dir, type Enemy, type GameState, type ShopItemId, type TileId } from '../src/core/types';
...
type Strategy = 'cautious' | 'pusher' | 'p01' | 'p02' | 'planner' | 'blind' | 'scatter' | 'linker' | 'smith' | 'wall' | 'mason' | 'ranger' | 'p01n' | 'patroller' | 'bare';
const ALL_STRATEGIES: Strategy[] = ['cautious', 'pusher', 'p01', 'p02', 'planner', 'blind', 'scatter', 'linker', 'smith', 'wall', 'mason', 'ranger', 'p01n', 'patroller', 'bare'];
const DRIFT_CAP = 80;
...
const HP_RETREAT_THRESHOLD: Record<Strategy, number> = { cautious: 0.3, pusher: 0.25, p01: 0.15, p02: 0.45, planner: 0.25, blind: 0.25, scatter: 0.25, linker: 0.25, smith: 0.25, wall: 0.3, mason: 0.3, ranger: 0.3, p01n: 0.35, patroller: 0.3, bare: 0.25 };
/** 交戦域（隣接超の敵をどこまで追って戦うか）。cautious/pusherは従来のswitch式を維持し、p01/p02はレビュー記載値を使う */
const ENGAGE_RANGE: Record<Strategy, number> = { cautious: 2, pusher: 5, p01: 6, p02: 2, planner: 5, blind: 5, scatter: 5, linker: 5, smith: 5, wall: 2, mason: 2, ranger: 2, p01n: 6, patroller: 2, bare: 5 };
/** 前線拠点の建設余力（購入コストの何倍の所持金があれば建てるか） */
const OUTPOST_BUDGET_MULT: Record<Strategy, number> = { cautious: 1.6, pusher: 1.2, p01: 1.2, p02: 1.6, planner: 1.2, blind: 1.2, scatter: 1.2, linker: 1.2, smith: 1.2, wall: 1.6, mason: 1.6, ranger: 1.6, p01n: 1.2, patroller: 1.6, bare: 1.2 };
...
const TURRET_BUDGET_MULT: Record<Strategy, number> = { cautious: 1.3, pusher: 1.8, p01: 1.8, p02: 1.3, planner: 1.8, blind: 1.8, scatter: 1.8, linker: 1.8, smith: 1.8, wall: 1.3, mason: 1.3, ranger: 1.3, p01n: 1.8, patroller: 1.3, bare: 1.8 };
...
/** 020新規: タレットを既存obstacleと隣接する位置へクラスタ配置して連携を狙う戦略か。022新規: patrollerも同じ配置様式 */
function usesClusterTurretPlacement(strategy: Strategy): boolean {
  return strategy === 'linker' || strategy === 'smith' || strategy === 'wall' || strategy === 'mason' || strategy === 'patroller';
}
/** 020新規: 鑑定(appraisal)に投資し、ロット品質を見て選別配置する戦略か。p01は仮説検証志向のため利用する。022新規: patrollerも投資する */
function usesAppraisal(strategy: Strategy): boolean {
  return strategy === 'smith' || isP01(strategy) || strategy === 'mason' || strategy === 'patroller';
}
/** 022新規: 夜の帰還時、パトロール圏がカバーするレーン（recommendedReturnLane）へ意図的に寄る戦略か。
 * masonと投資（appraisal＋クラスタ配置）は同一で、この行動の有無だけを比較する */
...
function mineDirs(s: GameState, strategy: Strategy): Dir[] {
  if (strategy === 'pusher' || isP01(strategy) || strategy === 'planner' || strategy === 'blind' || strategy === 'scatter' || strategy === 'linker' || strategy === 'smith' || strategy === 'bare') {
...
function shopPriorityFor(strategy: Strategy): ShopItemId[] {
  ...
  if (strategy === 'mason' || strategy === 'patroller') return CAUTIOUS_APPRAISER_PRIORITY;
  if (strategy === 'scatter' || strategy === 'linker' || strategy === 'bare') return PUSHER_PRIORITY;
...
class Bot {
  ...
  decide(s: GameState): Action {
    ...
    if (inBaseRadius(s)) {
      ...
      // 逆に死蔵しないか）を検証する
      if (!neverBuildsTurrets(this.strategy) && s.player.turretsAtCurrentBase < s.player.maxTurretsPerBase) {
    ...
    if (s.phase === 'night') {
      ...
      // 022新規: patrollerは、タレットのパトロール圏がカバーするレーン(recommendedReturnLane)へ
      // まだ乗っていなければ先に寄る（帰路支援が効くレーンを意図的に通る）。安全（隣接敵なし）な
      // ときだけ寄り道し、危険が迫れば通常のbfs帰還を優先する
      if (usesPatrolRouting(this.strategy) && p.recommendedReturnLane !== null && p.recommendedReturnLane !== p.y && (!nearest || nearest.dist > 2)) {
        const moveDir: Dir = p.y < p.recommendedReturnLane ? 'down' : 'up';
        if (canAdvance(s, moveDir)) return { type: 'move', dir: moveDir };
      }
...
interface RunResult {
  ...
  /** 022新規: core Metrics.deathPhase（'night-field'/'day-siege'/'in-base'/'none'）。本仕様の主指標 */
  deathPhaseCore: DeathPhase;
...
for (const strategy of strategies) {
  ...
  const deadResults = results.filter((r) => r.deathPhaseCore !== 'none');
  const phaseCount = (p: DeathPhase) => deadResults.filter((r) => r.deathPhaseCore === p).length;
  console.log(
