...
export class Renderer {
  ...
  draw(s: GameState): void {
    ...
    ctx.fillText(
      `拠点防衛投資 Lv${defShop?.level ?? 0}（迎撃dmg${s.player.baseAutoDefenseDmg}/tick）${defShop?.nextCost !== null && defShop?.nextCost !== undefined ? `  次Q¥${defShop.nextCost}` : '  MAX'}`,
      6,
      hudY + 144,
    );
    ctx.fillText(
    ...
    ctx.fillText(
      '1攻撃 2機動 3耐久 4ドリル 5燃料 6採掘速度 7ランタン 8危険耐性 9積載 0テレポート解禁 Q拠点防衛',
      6,
      hudY + 178,
    );
