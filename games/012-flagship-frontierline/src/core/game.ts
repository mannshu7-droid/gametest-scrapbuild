...
export class Game {
  ...
  /** 危険度2種のヒント（combat/mining）を毎tick1回だけ更新する（008パターン#11） */
  ...
  private applyBuildOutpost(): void {
    ...
    const cost = Math.round(OUTPOST_BASE_COST * (1 + Math.max(0, bandAt(x)) * OUTPOST_BAND_COST_MULT));
    if (this.player.money < cost) return;
    this.player.money -= cost;
