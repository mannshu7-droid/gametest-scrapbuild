import { LANES, LENGTH } from '../core/game';
import { TILE, type GameState } from '../core/types';

const TILE_PX = 32;
const VIEW_W = 20; // x方向の可視マス数（フィールド長321に対してスクロール表示）
const HUD_PX = 130;
...
export class Renderer {
  ...
  draw(s: GameState): void {
    ...
    if (camLeft === 0) {
      ...
      ctx.lineTo(TILE_PX, LANES * TILE_PX);
      ctx.stroke();
    }
    ...
    ctx.fillText(
      `tick ${s.tick}  maxDistance ${s.metrics.maxDistance}  oreMined ${s.metrics.oreMined}  score ${s.metrics.score}${s.player.teleportUnlocked ? '  [T]teleport可' : ''}`,
      8,
      hudY + 72,
    );
