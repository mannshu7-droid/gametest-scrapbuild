 ...
 * 1〜4=ショップ購入（ウェイステーション滞在中のみ意味を持つ）, Z/X=ルート選択（安全/危険）
 */
export class Input {
  ...
  constructor() {
    window.addEventListener('keydown', (e) => {
      ...
      if ([' ', 'e', 'shift', 'z', 'x', '1', '2', '3', '4'].includes(k)) {
        this.queued.push(k);
        e.preventDefault();
      }
      this.pressed.add(k);
  ...
  poll(): Action {
    ...
    if (single === 'z') return { type: 'chooseRoute', route: 'safe' };
    if (single === 'x') return { type: 'chooseRoute', route: 'risky' };
