 ...
 * 操作: WASD/矢印=移動・採掘（向きも保持）, Space=攻撃（直近の向き）, E=範囲攻撃, Q=緊急離脱(dash),
 * F=支保工設置（直近の向き）, G=前線基地の建設（足元）, 1〜9=ショップ購入（地上・前線基地滞在中のみ意味を持つ）
 */
export class Input {
  ...
  constructor() {
    window.addEventListener('keydown', (e) => {
      ...
      if ([' ', 'e', 'q', 'f', 'g', '1', '2', '3', '4', '5', '6', '7', '8', '9'].includes(k)) {
        this.queued.push(k);
        e.preventDefault();
      }
      this.pressed.add(k);
  ...
  poll(): Action {
    ...
    if (single === 'f') return { type: 'build', dir: this.facing };
    if (single === 'g') return { type: 'outpost' };
